;(function() {//匿名函数，防止污染
	            ...
	            variable: function() {
	            	this.obj.face = {//表情包
	            		...
	            		'常用语': [
	            			...
	            			['请多提宝贵意见。', '请多提宝贵意见。'], 
	            			['不用客气，再见！', '不用客气，再见！'], 
	            			['还有什么需要我解释的？', '还有什么需要我解释的？'], 
	            			['您的需要就是我的职责。', '您的需要就是我的职责。'], 
	            			['有不懂的地方您尽管问。', '有不懂的地方您尽管问。'], 
	            			['我们愿意为您提供各种咨询服务。', '我们愿意为您提供各种咨询服务。'], 
	            			['我们的服务还有很多不足之处，请多指点。', '我们的服务还有很多不足之处，请多指点。'], 
	            			['请您放心，我们一定帮您解决，办好后我们通知您。', '请您放心，我们一定帮您解决，办好后我们通知您。'], 
	            			['对不起，这个问题我不太清楚，请稍等一下，我帮您问一下。', '对不起，这个问题我不太清楚，请稍等一下，我帮您问一下。'], 
	            			['刚才的误会，请您能谅解。', '刚才的误会，请您能谅解。'], //10
	            			['我可以将您的意见向领导反映，改进我们的工作。', '我可以将您的意见向领导反映，改进我们的工作。'], 
	            			['由于我们工作上的过失，给您带来了麻烦，真是对不起。', '由于我们工作上的过失，给您带来了麻烦，真是对不起。'], 
	            			['您提的意见很好，我们一定采纳您的意见，改进工作。', '您提的意见很好，我们一定采纳您的意见，改进工作。'], 
	            			['没关系，只要能使您满意，我们就尽力去做。', '没关系，只要能使您满意，我们就尽力去做。'], 
	            			['不客气，这是我们应该做的。', '不客气，这是我们应该做的。'], 
	            			['我有个问题想请教一下。', '我有个问题想请教一下。'], 
	            			['有没有人帮我回答一下问题？', '有没有人帮我回答一下问题？'], //20
	            			['请稍候，我们的相关技术正在帮您处理。', '请稍候，我们的相关技术正在帮您处理。'], 
	            			['方便给一下您的联系方式吗？', '方便给一下您的联系方式吗？'], 
	            			['您需要我们向您提供些什么服务呢？', '您需要我们向您提供些什么服务呢？']
	            		],
	            	}
	            		...
	            		switch(key) {
	            			case '云问表情':
	            				...
	            				for(var i=0; i<this.options.rowNum*this.obj.maxNum_y; i++) {
	            					...
	            					if(i < mood.length) {
	            						srcHtml = '<img src="'+ this.options.src + mood[i][1] +'.gif">';
	            						title = mood[i][0];
	            						mark = mood[i][0];
	            					}
	        ...
	        Faqrobot.prototype = {
	            ...
	            robotHtml: function(data) {
	                ...
	                if(data.webConfig) {//欢迎语结构
	                    ...
	                    //%%
	                    html = this.options.kfHtml[0].replace(/%aId%/g, aId).replace(/%cluid%/g, cluid).replace(/%kfPic%/g, this.robot.kfPic).replace(/%robotName%/g, this.robot.robotName).replace(/%helloWord%/g, data.webConfig.helloWord).replace(/%formatDate%/g, this.getFormatDate());
	            ...
	            replaceFace: function(data) {
	            	var result = data.match(/\/[\u4e00-\u9fa5]{1,3}/g),
	            		faceName = '',
	            		resultArr = [],
	            		faceArr = [
	            			['/傲慢', 'aoman'], 
	            	...
	            	if(result) {
	            		for(var i=0; i<result.length; i++) {
	            			for(var j=0; j<faceArr.length; j++) {
	            				if(result[i].indexOf(faceArr[j][0])>=0) {
	            					if(result[i].length>=faceArr[j][0].length) {
										data = data.replace(faceArr[j][0], '<img src="'+ this.options.prefix + this.options.faceSrc +'$FAQ$'+ faceArr[j][1] +'.gif">');
	            					}
	            				}
	            			}
	            		}
	            	}
					data = data.replace(/\$FAQ\$/g, '');
	            	return data;
	            },
	            //回答流程问题->s=getflw
	            askFlwQue: function() {
	    		...
	    		request: function(options) {
	    			...
	    			$.ajax({
	    			    url: encodeURI(('http://demo.faqrobot.net/servlet/AQ')),//...为基础地址
    ...
    ;MN(function() {//使能获取到加载完毕的DOM
        ...
        var $minichatCss = MN('<link rel="stylesheet" href="'+ prefix[0] +'/'+ prefix[1] +'/minichat/css/minichat.css"/>').appendTo('head');
    	...
    	function minichatJs() {
	    	...
	    	var FAQ = new Faqrobot({
	            ...
	            //jid: 0,//自定义客服客户图标
				//robotName: 'FaqRobot',//机器人名称
				logoUrl: 'images/logo.png',//logo地址
				logoId: 'MN_logoSrc',//logo地址
    			webNameId: 'MN_logoWord',//公司名称Id
	            //userInfoId: 'userInfoId',//用户信息Id
	            kfPic: 'images/robot.png',  //客服图标
	            khPic: 'images/user.png', //客户图标
                formatDate: '%hour%:%minute%:%second% %year%-%month%-%date%',//配置时间格式(默认10:42:52 2016-06-24)
