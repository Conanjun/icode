                    ...
                    if (data.list[0]) {
                        ...
                        for (var i = data.list.length - 1; i >= 0; i--) {
                            ...
                            // recordData += (data.list[i].question ? This.customHtml(This.replaceFace(data.list[i].question), data.list[i].dateTime) : '') + ((data.list[i].reply || '') ? This.robotHtml(_data) : '')
                             ...
                             var obj={
                              ansCon:(data.list[i].ansMsg || '').replace(/[\r\n]/g, ''),
                            ...
                            recordData += (data.list[i].askMsg ? This.customHtml(This.replaceFace(data.list[i].askMsg),data.list[i].askTime,data.list[i].askType) : '') + ((data.list[i].ansMsg || '') ? This.robotHtml(_data) : '')
                        }
      ...
      if (data.webConfig) {//欢迎语结构
        ...
        if (data.robotAnswer[index]) {
          ...
          if (data.robotAnswer[index].msgType == 'voice') {
            ...
            ansCon = '<p>若无法播放，请点击<a href="' + data.robotAnswer[index].ansCon + '" target="_blank">下载</a></p><br/>'
            ansCon += '<video src="' + data.robotAnswer[index].ansCon + '" controls="controls">您的浏览器不支持 video 标签。</video>'
          }
          //%%
        ...
        uploader.on( 'uploadSuccess', function( file, response ) {
          if($(".imgdivs").size()>=9){
            ...
            var picDivs='<div class="imgdivs col-xs-2" rel='+response.sendUrlMsg[0].url+'><div><img src="'+response.sendUrlMsg[0].url  +'"><div class="delRound"  onclick="$(this).parent().parent().remove()">+</div></div></div>';
              // var picDivs='<div class="imgdivs col-xs-2" rel='+response.sendUrlMsg[0].url+'><img src="'+response.sendUrlMsg[0].url  +'" /><p><a onclick="$(this).parent().parent().remove()">删除</a></p></div>';  
