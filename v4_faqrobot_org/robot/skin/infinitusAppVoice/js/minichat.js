                	...
                	if(data.robotAnswer[index]) {
	                    ...
	                    if(data.robotAnswer[index].gusList) {
    	                    if(!data.robotAnswer[index].relateList[0] && data.robotAnswer[index].gusList[0]) {
    	                        ...
    	                        if(gusList.length>0){
    								gusListHtml= data.robotAnswer[index].gusWords?(data.robotAnswer[index].gusWords.ydWords||'<p>您是否还关心以下问题：</p>'):'<p>您是否还关心以下问题：</p>';
    	                        	has_ydWords = true;
    							}
    			        ...
    			        if(data) {
							if(data.status) {//x
								if(data.status == -1) {//站点不存在/长时间离开
									...
									if(data.tspan=='2000' || data.message == '您长时间没有交互信息，请重新刷新访问.' || data.message=='您已在其他客户端登陆.' || data.message=='您长时间没有交互信息，请刷新页面...'){
										options.callback(data);
									}else {
    			        		    	This.showMsg(data.message);
    			        			}
