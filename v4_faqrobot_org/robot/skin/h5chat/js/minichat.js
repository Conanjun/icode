  ...
  Faqrobot.prototype = {
    ...
    robotHtml: function (data, index, hotQue) {//hotQue代表欢迎语下面的热点问题
      ...
      if (data.webConfig) {//欢迎语结构
        ...
        if (data.robotAnswer[index]) {
           ...
           */
          if(this.isiOS){
            var matchArray = ansCon.match(/<a[^>\/>]*href=[\'\"]([^\'\"]+\.(?:rar|zip|7z ))[\'\"][^>\/>]*(?:>|\/>)/i);
            if(matchArray){
              ansCon =  ansCon.replace(matchArray[1], matchArray[1] +'" class="imgClick');
            }
          }
    ...
    initLeaveMsg: function () {
      ...
      $.validator.addMethod("emailFlag",function(value,element,params){  
          ...
          return this.optional(element)||(emailreg.test(value.trim()));  
      },"请输入正确格式的邮箱！");
      function msgSubmit(){
