...
/*	function init_webLogo(data){
    ...
    function init_leadQuestions(data){
		...
		var allObj = '<div class="serv chat-ctn"><span class="serv-icon"><img src="'+ options.kfUrl +'" /></span><div class="serv-ctn wc-pre"><i class="triangle triangle1"></i><i class="triangle triangle2"></i><p class="say-word pre-say-word">'+data.webConfig.helloWord+'</p>';
		var leaveHtml='';
		...
		allObj += leaveHtml+'<span class="nameWrap"><span class="r2-name">'+options.robotName+'</span><i class="time">'+getTime()+'</i></div></div>';
  ...
  function createMyWordsHtml(msg,time){
	...
	function createRobotWordsHtmlItem(ansItem,time){
        ...
        if( typeof ansItem.aId!="undefined" && ansItem.aId!=0){
            ...
            var helpDiv=$('<div/>').addClass("helper_aid_"+aid).attr('cluid',cluid).appendTo(all_if_use).html('<span style="color:#999; font-size:12px;">以上内容对您有帮助吗？</span>');
   ...
   //字数统计
    ...
    function helpclick(){
		...
		$('.helper_aid_'+aid).text('感谢您的评价!').css('color','#bbb');
		var s_q_url='';
		if(usefull==1){
