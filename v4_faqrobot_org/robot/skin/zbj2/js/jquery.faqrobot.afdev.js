...
$.fn.extend({
	...
	function sayHello(data){
		...
		if(typeof data.webConfig.logoUrl!="undefined"){
			options.logoUrl=data.webConfig.logoUrl==''?"skin/zbj2/images/logo.png":data.webConfig.logoUrl;
		...
		//配置推荐信息
		init_recommend(data.chatFormSugLink);
        $("#"+options.sendBtn).click(function(){
            sendeMsg();
			
        });
        $("#"+options.clearScreen).click(function(){
			var welcomP= $("<div/>").addClass("serv chat-ctn");
	 ...
	 //显示框常见问题
	function init_guideQuestions(data){
	   if(data.guideQuestions && data.guideQuestions.length>0){     //topAsk
		   var renderTo=$("#left_content");
		   var serv=$('<div class="serv chat-ctn"></div>').appendTo(renderTo);
		   var span=$('<span class="serv-icon"></span>').appendTo(serv);
		   var kfUrl=$('<img src="'+ options.kfUrl +'" />').appendTo(span);
		   var wcpre=$('<div class="serv-ctn wc-pre"></div>').appendTo(serv);
		   var i=$('<i class="triangle triangle1"></i><i class="triangle triangle2"></i>').appendTo(wcpre);
		   
		   var p=$('<p class="say-word pre-say-word"></p>').appendTo(wcpre);
		   var text=$('<div id="askText"></div>').text('我猜您想问：').appendTo(p);
		   var dl=$('<dl></dl>').appendTo(p);
		   	for(var i in data.guideQuestions){
		   		var dt=$('<dt></dt>').appendTo(dl);
		   		var dtHtml=$('<a href="javascript:void(0)" title="'+data.guideQuestions[i].question+'" aid="'+data.guideQuestions[i].solutionId+'">'+data.guideQuestions[i].question+'</a>').appendTo(dt);
	            dtHtml.click(function(){
	                askQue($(this).attr("title"),$(this).attr("aid"));
	                return false;
	            }) ;
		   	}
		   	var nameWrap=$('<span class="nameWrap"><span class="r2-name">'+options.robotName+'</span><i class="time">'+getTime()+'</i></span>').appendTo(wcpre);
	   }
	};
	 ...
	 //流程问题的查看
    chatContDIv.on('click','.wflink',function() {
        $this = $(this);
	...
	function init_quickLink(quickLink){
		if(options.quickLink && options.quickLink.length>2 && quickLink && quickLink.length>0 ){
			...
			for(var i=0;i<quickLink.length;i++){
				html.push('<li ><a href="'+quickLink[i].linkUrl+'" target="_blank"><img src="'+quickLink[i].imageUrl+'" ></a></li>');
			}
			$('#'+options.quickLink+' ul').html(html.join(''));
		}
	}
	
	
	var len,$target,$up,$fkz,$down,wholeH;
	//推荐资讯
	function init_recommend(recommendata){
		if(options.recommend && options.recommend.length>2 && recommendata && recommendata.length>0 ){
			recommendata[0].content +='<span class="hot"><span>';
			recommendata[1].content +='<span class="hot"><span>';
			var html=[];
			len = recommendata.length;
			$target = $("#"+options.recommend);
			$up = $target.parent();
			$down = $target.find("dl");
			if(recommendata[0].type==0){
				for(var i=0;i<30;i++){
				//if(recommendata[i].type==0){
					for(var j=0;j<len;j++) {
						html.push('<dt><a href="'+recommendata[j].linkurl+'" target="_blank">'+recommendata[j].content+'</a></dt>');
						var a = 26*10*len;
						$down.css({"height":a});
						//console.log($(".fuzb2 dl").outerHeight());
					}
					
					
					/**********/
					$up.css({"overflow":"hidden","position":"relative"});
					$target.css({"position":"relative","overflow":"hidden","height":"100%"});
					$down.css({"position":"absolute","top":"0","left":"0"});
					wholeH = $(".fuzb2 dl").outerHeight();
				}
			}else{
					for(var i=0;i<len;i++){
					$up.css({"overflow":"auto"});
					//$("#"+options.recommend).css({"height":"100%"});
					//$("#"+options.recommend).find("dt").height(fhzbHeight/3).css({"border":"1px solid red"});
					html.push('<div><dt class="fkz"><a href="'+recommendata[i].linkurl+'" target="_blank"><img class="a" src="'+recommendata[i].content+' "></a></dt></div>');
				}
			}
				
			
			$('#'+options.recommend+' dl').html(html.join(''));
			//$(".a").css({"width":"100%"});
			$fkz = $(".fkz");
			//setH();
			...
			$target.on("mouseout", function() {
				xTimer = setInterval(function() {
					$down.css({"top":"-=.2px"});
					if(parseInt($down.css("top")) <-wholeH/10) {
						$down.css({"top":"0"});
					};
				}, 10);
			});
			...
			//
			/*var xmove = function() {
				$down.animate({"top":-wholeH/2},5000*len,"linear", function() {
					xmove();
					$down.css({"top":0});
				});
			}*/
		...
		}else if(options.recommend && options.recommend.length>3 && recommendata && recommendata.length>0 ){
			recommendata[2].content +='<span class="hot"><span>';
		}
	}
...
/*	function init_webLogo(data){
    ...
    function init_leadQuestions(data){
		...
		var allObj = '<div class="serv chat-ctn"><span class="serv-icon"><img src="'+ options.kfUrl +'" /></span><div class="serv-ctn wc-pre"><i class="triangle triangle1"></i><i class="triangle triangle2"></i><p class="say-word pre-say-word">'+(data.helloWord || data.webConfig.helloWord)+'</p>';
		var leaveHtml='';
		if(data.leaveQue && data.leaveQue.length>1){
			leaveHtml = '<div class="MN_leaveQueTop"><span>这里有您的留言，</span>请<span class="MN_lookLeaveQue">点击查看</span></div>';
			for(var i=0; i<data.leaveQue.length; i++) {
				leaveHtml += '<div class="MN_leaveQueBottom"><div class="MN_leaveQueItem"><div class="">问题'+ (data.leaveQue.length==1?'':i+1) +'：'+ data.leaveQue[i].question +'</div><div class="">答案：'+ data.leaveQue[i].answer +'</div></div></div>';
			}
        }
		allObj += leaveHtml+'<span class="nameWrap"><span class="r2-name">'+options.robotName+'</span></div></div>';//<i class="time">'+getTime()+'</i>
	   ...
	   chatContDIv.html(allObj);
		...
		tempUrl=tempUrl.substring(tempUrl.indexOf("=")+1,tempUrl.length).split("&");
		var systest=new RegExp('sysNum');
		if(!systest.test(tempUrl)){
			if(tempUrl[1]&&tempUrl[1].split('=')[0]=="question"){
				askQue(decodeURI(tempUrl[1].split('=')[1]));
			}
		}
		//^^^^^^^^^^^^^^^^^^^
		setScroll();
    }
	chatContDIv.on('click', '.MN_lookLeaveQue', function() {
    ...
    function formatQueULList(targetId,list,ulclass,maxCount,maxWords){
        ...
        for (var i=0; i < max; i++) {
			...
			//list[0].question += '<span class="hot"></span>';
            if (!list[i]){
                continue;
            }
			//console.log(list[0].question)  字符串
			...
			//var hotque=list[0].question;
			//hotque += '<span class="hot"><span>';

			if(i==0||i==1){
			var pchild = $("<span/>")
				.appendTo(lielement[0])
				.attr("class","hot")
        }
			if(i==2){
				var schild = $("<span/>")
	...
	function askQue(msg,slutionId){
        if(msg && msg.length>0){
        	...
        	if(msg.split('“').length > 1){
        		msg = msg.split('“')[0]+''+msg1.split('”')[0]+''+msg1.split('”')[1];
        	}*/
  ...
  function createMyWordsHtml(msg,time){
		var myword='<div class="customer chat-ctn"><span class="serv-icon"><img src="'+ options.khUrl +'" /></span><div class="serv-ctn"><i class="triangle triangle1"></i><i class="triangle triangle2"></i><p class="say-word">'+msg+'</p><i class="time">我</i></div> </div><div class="cl"></div>';
        return myword;
    }
    ...
    function showRobotWordsToForm(data){
	    ...
	    chatContDIv.append(formattedCont);
    	//^^^^^^^^^^^^^^^^^^^^^
		for(var i = 0;i < $('#left_content .serv-ctn a').length;i++){
			if($('#left_content .serv-ctn a').eq(i).attr('href')){
				if($('#left_content .serv-ctn a').eq(i).attr('href').indexOf('s=needperson') > -1){
					$('#left_content .serv-ctn a').eq(i).addClass('faqevent')
				}
			}
		}
		setScroll();
	...
	function createRobotWordsHtmlItem(ansItem,time){
		...
		var img = $("<img />").attr({"src":options.kfUrl}).appendTo(all_logo);
		...
		all_nameWrap.append('<span class="r-name">'+options.robotName+'</span>');
        ...
        if( typeof ansItem.aId!="undefined" && ansItem.aId!=0){
            ...
            var helpDiv=$('<div/>').addClass("helper_aid_"+aid).appendTo(all_if_use).html('<span style="color:#999; font-size:12px;">以上内容对您有帮助吗？</span>');
   ...
   //字数统计
	 ...
	 function init_unsolvedReason(data){
		 ...
		 if(data && data.length>0){
			tmpCon.push('<form class="unsolved">');
		 	tmpCon.push('<p>非常抱歉没能解决您的问题，请反馈不满意原因，我们会根据您的反馈进行优化与完善！</p>');
	  ...
	  chatContDIv.on('click','.subreasonForm',function(){
		  ...
		  $.ajax({
				...
				success:
					function(data){
						if(data.status==0){
							tmoObj.find('.ishaveyong').css('visibility','visible').html('<div style="color:#666;">呜呜呜~不好意思啦,'+options.robotName+'会日夜刻苦学习,希望下次帮您排忧！</div>');
    ...
    function helpclick(){
            ...
            if(usefull==1){
				...
				if(options.unsolvedReason && options.unsolvedReason.length>0){
					showReason(aid);
				}
            }
            request(s_q_url,function(data){
				 $('.helper_aid_'+aid).text(data.robotAnswer[0].ansCon || data.message).css('color','#bbb');
			});
        }
    //****************************************公用方法******************************************//
...
$.FaqRobot.defaults = {
	...
	kfUrl:'skin/zbj2/images/servIcon.png',  //客服图标
	khUrl:'skin/zbj2/images/userIcon.png', //客户图标
	//>>>>>>>>>>>>>>
	quickLink:'quickLink',
	recommend:'recommend',					//推荐资讯
	sourceId:0,                //区分访客来自网页还是微信
	unsolvedReason:'',         //未解决问题选择不满意原因
    //隐藏的自定方法
    formatWebLog:false,
