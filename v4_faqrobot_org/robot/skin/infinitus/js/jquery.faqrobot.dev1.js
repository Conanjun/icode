...
$.fn.extend({
	...
	function sayHello(data){
		...
		if(typeof data.webConfig.logoUrl!="undefined"){
			options.logoUrl=data.webConfig.logoUrl==''?"skin/eur-go/images/logo.png":data.webConfig.logoUrl;
	...
	function init_quickLink(quickLink){
		if(options.quickLink && options.quickLink.length>2 && typeof quickLink!="undefined" && quickLink.length>0 ){
			...
			for(var i=0;i<quickLink.length;i++){
				html.push('<li><a href="../../eur-go - 拷贝/js/'+quickLink[i].linkUrl+'" target="_blank"><img src="../../eur-go - 拷贝/js/'+quickLink[i].imageUrl+'"></a><br><div>'+quickLink[i].name+'</div></li>');
			}
    ...
    function init_leadQuestions(data){
		...
		var all_logo=$('<p/>').addClass('i').html('<img src="../../eur-go - 拷贝/js/skin/eur-go/images/fi1.png"  width="50">').appendTo(allObj);
		...
		if(options.baseInfoDiv && options.baseInfoDiv.length>2){
			if(typeof data.webConfig!="undefined"){
			  ...
			  if(typeof data.webConfig.webSite!="undefined"){
					cardhtml2+='<dt>网址：<a href="../../eur-go - 拷贝/js/'+data.webConfig.webSite+'" target="_black">'+data.webConfig.webSite+'</a></dt>';
			  }
			}
				...
				for(var i=0;i<data.advList.length;i++){
					...
					if(style==0){
						...
						if(data.advList[i].value.indexOf('<')==0 && data.advList[i].value.indexOf('>')>0){
							...
							cardhtml2+='<dt>'+data.advList[i].name+'：<br><img src="../../eur-go - 拷贝/js/'+data.advList[i].value+'">';
						}
						...
						if(data.advList[i].value.indexOf('<')==0 && data.advList[i].value.indexOf('>')>0){
							...
							cardhtml2+='<dt>'+data.advList[i].name+'：<a href="http://wpa.qq.com/msgrd?v=3&uin='+data.advList[i].value+'&site=qq&menu=yes" target="_blank"><img alt="'+data.advList[i].name+'" src="../../eur-go - 拷贝/js/images/Service1.png"></a>';
						}
					}
    ...
    function createMyWordsHtml(msg,time){
		var myword='<div class="msgright"><p class="ri"><img width="55" src="../../eur-go - 拷贝/js/skin/eur-go/images/fi2.jpg" style="margin-left:5px;"></p><div class="rmsgcontent"><p class="spacing15"></p><div class="metext"><span class="s1">我：</span><span>'+msg+'</span>  <br><span class="s2">'+time+'</span><p class="spacing5"></p></div><div class="ishaveyong"><p class="spacing5"></p></div><p class="spacing"></p></div></div><p class="spacing15"></p>';
        return myword;
    }
	...
	function createRobotWordsHtmlItem(ansItem,time){
        ...
        if( typeof ansItem.aId!="undefined" && ansItem.aId!=0){
            ...
            $('<a/>').addClass("robot_review_yes").attr("helptag",1).attr("aid",aid).click(helpclick).html('<span></span>满意').appendTo(helpDiv);
            $('<a/>').addClass("robot_review_no").attr("helptag",0).attr("aid",aid).click(helpclick).html('<code></code>不满意').appendTo(helpDiv);
        }else{
        }
        //相关问题(右侧应该配置div的ID)
        if( typeof ansItem.aId!="undefined" && ansItem.aId!=0){
   ...
   //字数统计
    ...
    function helpclick(){
            ...
            var s_q_url='';
            if(usefull==1){
                s_q_url=options.basePath+'AQ?s=addufc&aId='+aid+'&sysNum='+options.sysNum;
            }else{
                s_q_url=options.basePath+'AQ?s=addulc&aId='+aid+'&sysNum='+options.sysNum;
            }
            request(s_q_url);
        }
    //****************************************公用方法******************************************//
	...
	if(options.package && options.package.length>2){
		$("#"+options.package).click(function(){
			searchPackage();
		});
	}
	if(options.calcFees && options.calcFees.length>2){
		$("#"+options.calcFees).click(function(){
			askQue('计算运费',166233);
		});
	}
	...
	function Evaluation(){
		if($('.feedback').length>0){
			OAnswer.push('<div class="msgleft"><p class="i"><img width="50" src="../../eur-go - 拷贝/js/skin/eur-go/images/fi1.png"></p><div class="msgcontent"><p class="spacing"></p><div class="kftext"><div><span class="s1">'+options.robotName+'</span>：<p>感谢您的支持，您已经做过评价了！</p><span class="s2">'+getTime()+'</span></div><p class="spacing5"></p><div class="ishaveyong"><p class="spacing5"></p><span></span></div></div><p class="spacing"></p></div></div><p class="spacing15"></p>');
		}else{
		OAnswer.push('<div class="msgleft satisfied">');
		OAnswer.push('<p class="kissub"></p>');
		OAnswer.push('<p class="i"><img src="../../eur-go - 拷贝/js/skin/eur-go/images/fi1.png" width="50" /></p>');
	...
	function speak(){
		if($('.packge').length>0){
			$('.packge').find('.enwrap').remove();
		}
		OAnswer.push('<div class="msgleft body packge">');
		OAnswer.push('<p class="kissub"></p>');
		OAnswer.push('<p class="i"><img src="../../eur-go - 拷贝/js/skin/eur-go/images/fi1.png" width="50" /></p>');
		...
		OAnswer.push('<span class="s1">'+options.robotName+'：包裹追踪</span>');
		OAnswer.push('<p class="spacing"></p>');
		OAnswer.push('<div class="enwrap">');
		OAnswer.push('<div class="wrapHead" >');
		OAnswer.push('<span>运单跟踪查询/INQUIRY</span></div>');
		OAnswer.push('<div class="searchInput"><input type="text" rel="请输入DHL运单号" value="请输入DHL运单号"><input type="button"  class="DHLSearch"></div>');
		OAnswer.push('<div class="searchInput"><input type="text" rel="请输入PostNL运单号" value="请输入PostNL运单号"><a href="https://www.internationalparceltracking.com/Default.aspx#/search/BARCODE/COUNTRY/POSTALCODE" target="_blank"><input type="button"></a></div>');
		OAnswer.push('<div class="searchInput"><input type="text" rel="请输入邮政EMS运单号" value="请输入邮政EMS运单号"><a href="http://www.ems.com.cn/mailtracking/you_jian_cha_xun.html" target="_blank"><input type="button"></a></div>');
		OAnswer.push('<div class="searchInput"><input type="text" rel="请输入邮政普邮运单号" value="请输入邮政普邮运单号"><a href="http://intmail.11185.cn/zdxt/yjcx/" target="_blank"><input type="button"></a></div>');
		OAnswer.push('</div>');
		OAnswer.push('<span class="s2">'+getTime()+'</span><p class="spacing"></p>');
		OAnswer.push('</div>');
		OAnswer.push('<p class="spacing"></p>');
		OAnswer.push('</div>');
		OAnswer.push('</div>');
	...
	function showMes(name, content) {
		...
		OAnswer.push('<p class="ri"><img src="../../eur-go - 拷贝/js/skin/eur-go/images/fi2.jpg" width="55" style="margin-left:5px;"/></p>');
	...
	//反馈建议（西奥）
	function fbForm(){
		var telCur=$('#'+options.fadebackForm+' input[name=telNum]').val();
		var titleCur=$('#'+options.fadebackForm+' input[name=title]').val();
		var nameCur=$('#'+options.fadebackForm+' input[name=userName]').val();
		...
		if(!(/^1[3|5][0-9]\d{4,8}$/.test(telCur)) && telCur!=''){ 
			tips('请输入有效的手机号码');
			return;
		}
		var fadeUrl=options.basePath+'AQ?s=advice&sysNum='+options.sysNum;
		$.ajax({
			type:'post',
			datatype:'json',
			cache:false,//不从缓存中去数据
			url:encodeURI(fadeUrl),
			data:$('#'+options.fadebackForm).serialize(),
			success:
			function(data){
				if(data.status==0){
					$('#'+options.fadebackForm+' input').val(''); 
					$('#'+options.fadebackForm+' textarea').val('请输入您宝贵的意见和建议……'); 
				}
				tips(data.message);
			}
		});
	...
	//tips
	function tips(value){
		var obj=$('#'+options.fadebackForm+' p').find('span');
		obj.html(value);
		obj.delay(3000).fadeTo("slow", 0.2).fadeTo("slow", 1).fadeOut(2000,function(){
			obj.html('');
		}).stop();
	}
	if(options.fadebackBtn && options.fadebackBtn.length>2){
		$('#'+options.fadebackBtn).live('click',function(){
			fbForm();
		})
    }
};
...
$.FaqRobot.defaults = {
	...
	quickLink:'quickLink',
fadebackForm:'fadebackForm',//反馈建议
	fadebackBtn:'btnSub',//提交按钮
    //隐藏的自定方法
    formatWebLog:false,
    formatBaseQueCount:false,
    showErrorMsg:false,
	package:'package',    //欧洲GO包裹查询
	calcFees:'calcFees' , //运费计算
	
	teachMetalk:'teachMetalk',    		//eur-go中教我说话id，不用可以去掉
	fadeBackIdLink:'fadeBackIdLink'     //eur-go中满意度评价id，不用可以去掉
};
})(jQuery);
