//;(function() {//匿名函数，防止污染
    ...
    */  
    //;(function($, window, document, undefined) {
	    ...
	    //上传方法  
	    function H5_upload(url, maxNum, $trigger, $ctn, startcall, callback, type, typecall) {//$trigger-触发的input $ctn-添加到的父级 callback-请求成功回调  type-允许上传的文件格式
	        //自动上传    
			                ...
			                for(var i=0,len=val.length; i<len; i++) {  
			            		...
			            		if(type[0]) {
			                		for(var j=0,typeLen=type.length; j<typeLen; j++) {
			                			if(/\.gif|jpeg|bmp|jpg|png/i.test(val[i].name)) {
			                				arr[j] = 1;
			                			}else {
			                				arr[j] = 0;
			                			}
    ...
    ;(function($, window, document, undefined) {
        ...
        var plugName = "faqrobot",
            defaults = {
            	...
            	getCallback: function(answer) {},//获取到答案后的回调
        ...
        Faqrobot.prototype = {
            ...
            historyRecord: function() {
        		...
        		This.request({
        			...
        			callback: function(data) {
        	    		if(data.status || data.message=='您已在其他客户端登陆.' || data.message=='您长时间没有交互信息，请重新刷新访问.' || data.message=='您长时间没有交互信息，请刷新页面...') {
							This.request({
								params: {
									s: 'p',
									jid: This.options.jid,
                	...
                	if(data.robotAnswer[index]) {
	                    ...
	                    if(data.robotAnswer[index].gusList) {
    	                    if(!data.robotAnswer[index].relateList[0] && data.robotAnswer[index].gusList[0]) {
    	                        ...
    	                        if(gusList.length>0){
    								gusListHtml= data.robotAnswer[index].gusWords?(data.robotAnswer[index].gusWords.ydWords||'<p>您反馈的问题已收到，会纳入后期优化中，感谢您参与APP的测试。您是否还关心以下问题：</p>'):'<p>您反馈的问题已收到，会纳入后期优化中，感谢您参与APP的测试。您是否还关心以下问题：</p>';
    	                        	has_ydWords = true;
    							}
	                    ...
	                    var ansCon = data.robotAnswer[index].ansCon.replace('转人工', '<a class="welcomeWords" href="javascript:;">转人工</a>');
                    	//%%
                ...
                if(question) {//问题不为空
                    ...
                    if(question.indexOf('%我要发文件%')+1) {//发文件
                    	...
                    	if(This.robot.html) {// 当 This.robot.html 不为空时，走模拟答案
                    		...
                    		This.options.getCallback(This.getCurrectWords(This.robotHtml(data)));//获取到答案后的回调
                    		This.recommendQue(data);//推荐问题
                    	}else {
							var extraParams = {},
							imgUrl = question.match(/https?[^>]*?\.(png|jpg|bmp|jpeg|gif)/g);
							if(imgUrl) {
                    		...
                    		This.request({
                    		    ...
                    		    callback: function(data) {
									if(data.status || data.tspan=='2000' || data.message=='您已在其他客户端登陆.' || data.message=='您长时间没有交互信息，请重新刷新访问.' || data.message=='您长时间没有交互信息，请刷新页面...'){
										This.request({
		        		    				params: {
		        		    		            s: 'p',
		        		    		            jid: This.options.jid,
            ...
            askQueBack: function(data) {
				...
				if(robotHtml.indexOf('welcomeWords')+1) {
					This.robot.guide = false;// 退出引导
				}
		    	if(hasCtn) {
	    			This.$obj.$chatCtnId.append(robotHtml);//添加机器人的话
					This.options.getCallback(This.getCurrectWords(robotHtml));//获取到答案后的回调
		    	}
	    		This.recommendQue(data);//推荐问题
                ...
                This.$obj.$chatCtnId.on('click.FA', '.MN_reasonSend', function() {
                    ...
                    This.request({
                        ...
                        callback: function(data) {
                        	if(!data.status) {
                        		$form.remove();
                            	$This.parents('.MN_helpful').html(window.addulcTip.replace('转人工', '<a class="welcomeWords" href="javascript:;">转人工</a>') || '感谢您的评价！');
                        	}
                        }
                    });
                });
            ...
            upFile: function() {
            	...
            	H5_upload('../../servlet/appChat?s=uf', this.options.upFileModule.maxNum, $file, this.$obj.$chatCtnId, function(ran) {
												...
												if(data.robotAnswer[0]) {
													for(var i=0,len=data.robotAnswer.length; i<len; i++) {
														if(data.robotAnswer[i].ansCon) {
															...
															This.options.getCallback(This.getCurrectWords(This.robotHtml(data, i)));//获取到答案后的回调
    		...
    		request: function(options) {
    			...
    			options = $.extend({}, defaults, options);
    			formData = $.extend({}, This.formatSeriData(location.href), This.formatSeriData(decodeURIComponent((options.$formObj.serialize()))), options.dataObj);
    			...
    			$.ajax({
    			    url: encodeURI(options.prefix || (This.options.prefix + 'servlet/appChat')),//...为基础地址
    			        ...
    			        if(data) {
							if(data.status) {//x
    			        		if(data.status == -1) {//站点不存在/长时间离开
    			        			if(data.message == '您长时间没有交互信息，请重新刷新访问.' || data.message=='您已在其他客户端登陆.' || data.message=='您长时间没有交互信息，请刷新页面...') {
