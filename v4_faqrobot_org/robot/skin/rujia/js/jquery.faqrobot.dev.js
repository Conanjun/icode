...
$.fn.extend({
	...
	function sayHello(data){
		...
		if(typeof data.webConfig.logoUrl!="undefined"){
			options.logoUrl=data.webConfig.logoUrl==''?"skin/rujia/image/logo.png":data.webConfig.logoUrl;
    ...
    $('.wflink').live('click',function() {
	 ...
	 //未解决的原因
	 function showReason(aid,uid){
		var tmpObj=$('.helper_aid_'+aid);
		tmpObj.siblings('.questionR_txt').after(init_unsolvedReason(options.unsolvedReason));
		tmpObj.siblings('.unsolved').find('.subreasonForm').attr('cluid',uid);
	  ...
	  chatContDIv.on('click','.subreasonForm',function(){
		  ...
		  var tmoObj=$(this).parents('.robot_questionR_qp');
		  ...
		  $.ajax({
				...
				success:
					function(data){
						if(data.status==0){
							tmoObj.find('.robot_review2').css('visibility','visible').html('感谢您的反馈！');
    ...
    function init_leadQuestions(data){
        var allObj = $('<div/>').addClass("robot_question").html('<dl class="robot_questionL"><img src="skin/rujia/image/1_03.png"></dl>');
	...
	function createRobotWordsHtmlItem(ansItem,time){
        ...
        if( typeof ansItem.aId!="undefined" && ansItem.aId!=0){
            var aid=ansItem.aId;
			var cuid=ansItem.cluid;
            rightCon_helpWords.addClass("helper_aid_"+aid);
            $('<a/>').addClass("robot_review_yes").attr("helptag",1).attr("aid",aid).attr("uid",cuid).click(helpclick).html('<span></span>满意').appendTo(rightCon_helpWords);
            $('<a/>').addClass("robot_review_no showReason").attr("helptag",0).attr("aid",aid).attr("uid",cuid).click(helpclick).html('<code></code>不满意').appendTo(rightCon_helpWords);
...
$.FaqRobot.defaults = {
	...
	kfUrl:'skin/rujia/image/1_03.png',  //客服图标
	khUrl:'skin/rujia/image/robot_askicon.png', //客户图标
	quickLink:'quickLink',				//快捷服务
	unsolvedReason:'',         			//未解决问题自定义
