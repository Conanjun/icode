;$(function() {
		...
		servLeaveMsg: function() {
			...
			*/
			...
			function msgSubmit(){
				...
				var messageCompany=$("#leaveMsgForm input[name=qq]").val().trim()//公司
    ...
    $('#a,#sendVoiceQue').on('touchstart',function(e){
        e.preventDefault();
        startRecord(e)
    })
    $('#a,#sendVoiceQue').on('touchend',function(e){
        e.preventDefault();
        stopRecord(e)
    })
    $('#a,#sendVoiceQue').on('touchmove',function(e){
        e.preventDefault();
        touchMove(e)
    })
    var START = '',
