;$(function() {
    ...
    function set_chatScroll_height() {
        var winW = $(window).width(),
            winH = $(window).height();
        $('html').css('fontSize', winW<220 ? winW : 220);
    ...
    var Face = $('.textarea').face({
        ...
        ctnAttr: ['0px', '88px', '40px', '40px'],//[left bottom width height] 表情框相对targetEl位置和里面的表情格子宽高  要写单位
        triggerEl: $('.faceBtn'),//触发按钮(不存在则自己生成，不要由a包裹)
        targetEl: $('.editShow'),//父级参照物(用于appendTo和定位)
        hideAdv: true,//是否隐藏广告
        callback: function() {
            $('.editHide').hide();
            $('.sendBtn').show().siblings().hide();
    ...
    $('.textarea').autocomplete({
        ...
        posAttr: ['0rem', '0.4rem'],//外边框的定位[left bottom]
    ...
    var FAQ = new Faqrobot({
        ...
        logoUrl: 'robot/skin/dang/images/logo@2x.png',//logo地址 ----------
        logoId: 'logo',// ----------
        webNameId: 'MN_logoWord',//公司名称Id
        //userInfoId: 'userInfoId',//用户信息Id
        kfPic: 'robot/skin/dang/images/robot.png',  //客服图标
        khPic: 'robot/skin/dang/images/user.png', //客户图标
                ...
                setTimeout(function() {
                    if(!data.status) {
                        if(+$('[name=level]:checked').val()) {//满意
                            layer.msg('<div '+ (MN_Base.ieVersion()==7?'style="color: #fff;"':'') +'>能帮到您小当好开心的呢，我会继续努力的哦^_^</div>', {
                                area: '200px'
                            });
                        }else {
                            layer.msg('<div'+ (MN_Base.ieVersion()==7?'style="color: #fff;"':'') +'>没有让您满意小当感到万分心痛，我会继续学习~</div>', {
                                area: '200px'
                            });
                        }
                    }
    ...
    $('.faceBtn').hover(function() {
        $(this).attr('src', 'skin/dangpc/images/expression@2xhover.png')
    }, function() {
        $(this).attr('src', 'skin/dangpc/images/expression@2x.png')
    })
