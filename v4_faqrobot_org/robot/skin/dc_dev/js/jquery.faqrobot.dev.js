...
$.fn.extend({
	...
	function sayHello(data){
		...
		//设置机器人的名字
		if(data.webConfig.webName != null||data.webConfig.webName != ''){
			$('#titleRobot').html(data.webConfig.webName);
		}
		//设置机器客服名字
		...
		if(typeof data.webConfig.logoUrl!="undefined"){
			options.logoUrl=data.webConfig.logoUrl==''?"skin/dc_dev/image/serv_red.png":data.webConfig.logoUrl;
 	...
 	function init_quickLink(quickLink){
		if(options.quickLink && options.quickLink.length>2 && typeof quickLink!="undefined" && quickLink.length>0 ){
			...
			for(var i=0;i<quickLink.length;i++){
				html.push('<a class="a'+ (i+1) +'" href="'+ quickLink[i].linkUrl +'"><img src="'+ quickLink[i].imageUrl +'" /><p>'+ quickLink[i].name +'</p></a>')
			}
			$('#'+options.quickLink+' ul').empty().append(html.join('')).append('<a class="vcode" href="#"><img src="skin/dc_dev/image/vcode.jpg" /><p>扫一扫 码上有礼</p></a>');
		}
	}
	
    //更改引导问题
    function init_leadQuestions(data){
        var allObj = $('<div/>').addClass("robot_question").html('<dl class="robot_questionL"><img src="'+options.logoUrl+'"></dl>');

        var all_r = $('<dl/>').addClass("robot_questionR").appendTo(allObj);
        
        var kfName = $('<div/>').addClass('MN_kfName').appendTo(all_r)

        var a_rightCon = $('<div/>').addClass("robot_questionR_qp").append('<i class="triangle triangle1"></i><i class="triangle triangle2"></i>').appendTo(all_r);
        ...
        if(options.chatContDiv && options.chatContDiv.length>2 && typeof data.leaveQue!="undefined" && data.leaveQue!=null && data.leaveQue.length>1 ){
            ...
            rightCon_robotWords.html(data.helloWord||data.webConfig.helloWord);
            kfName.html(data.webConfig.robotName)
		}
    ...
    window.askQue = function(msg,slutionId){
		options.curQue=msg;
        if(msg && msg.length>0){
    ...
    function createMyWordsHtml(msg,time){
        var myword='<div class="robot_ask"><div class="MN_khName">我</div><dl class="robot_askL"><i class="triangle triangle1"></i><i class="triangle triangle2"></i><dt>'+msg+'<span></span></dt><dd>'+time+'</dd></dl><dl class="robot_askR"><img src="'+options.khUrl+'" class="fixpng"/></dl></div>';
        return myword;
    }
    ...
    function createRobotWordsHtml(data,time){
		...
		for(var i=0;i<data.robotAnswer.length;i++){
			if(data.robotAnswer[i].ansCon!=''||data.robotAnswer[i].gusList!=''){
	...
	function createRobotWordsHtmlItem(ansItem,time){
        ...
        var kfName = $('<div/>').addClass('MN_kfName').html(options.robotName).appendTo(all_r)
        ...
        if( typeof ansItem.aId!="undefined" && ansItem.aId!=0){
            ...
            $('<a/>').addClass("robot_review_yes").attr("helptag",1).attr("aid",aid).attr("uid",cluid).click(helpclick).html('<span></span>满意').appendTo(rightCon_helpWords);
            $('<a/>').addClass("robot_review_no").attr("helptag",0).attr("aid",aid).attr("uid",cluid).click(helpclick).html('<code></code>不满意').appendTo(rightCon_helpWords);
		...
		var ansCon  = ansItem.ansCon;
		if(ansItem.ansCon!="" && ansItem.ansCon.length>0){
            var listAll = '';
			if (ansItem.msgType == 'voice') {
				//语音答案
				ansCon = '<p>若无法播放，请点击<a href="' + ansItem.ansCon + '" target="_blank">下载</a></p><br/>'
				ansCon += '<audio src="' + ansItem.ansCon + '" controls="controls">您的浏览器不支持 audio 标签。</audio>'
			} else if (ansItem.msgType == 'image') {
				//图片答案
				ansCon = '<img src="' + ansItem.ansCon + '" class="imgBox">'
			} else if (ansItem.msgType == 'video') {
				//视频答案
				ansCon = '<p>若无法播放，请点击<a href="' + ansItem.ansCon + '" target="_blank">下载</a></p><br/>'
				ansCon += '<video src="' + ansItem.ansCon + '" controls="controls" style="max-width:100%;">您的浏览器不支持 video 标签。</video>'
			}
            rightCon_robotWords.html(ansCon);
        }
        ...
        rightCon_robotWords.html(replace_em(ansCon));
        return allObj;
    }
    //下线的事件
    function offLine(){
   ...
   //字数统计
	...
	function replace_em(str){
		str = str.replace(/\[em_([0-9]*)\]/g,'<img src="/onlineMonitor/monitor/arclist/$1.gif" border="0" />');
		return str;
	}
    initRobotBaseInfo();
};
...
$.FaqRobot.defaults = {
	...
	logoUrl:"skin/robotVedio/image/logo.png",
	...
	kfUrl:'skin/dc_dev/image/serv_pink.png',  //客服图标
	khUrl:'skin/dc_dev/image/robot_askicon.png', //客户图标
