...
var functions = {
    ...
    getUserDetail: function(uid) {
        return functions.callApi(_MIDEA_USER, 'vcard', [uid]);
    },
    // cordova.exec(function (success) {
    //      }, function (error) {
    //                     alert("Error: " + error);
    //      }, "MideaUser", "vcard", [self.engineer_Id]);
    /**
     * 显示菜单
     * @return {*|promise}
     */
        ...
        return new Promise(function(resolve, reject) {
            if (navigator.camera) {
                try {
                    ...
                    cameraOptions.destinationType = navigator.camera.DestinationType.DATA_URL; // 返回的图片数据格式  FILE_URI    DATA_URL
