  ...
  Face.prototype = {
    ...
    variable: function () {
      this.obj.face = {//表情包
        ...
        '自如表情': [  /*TaskId:  408 自如页面定制 添加自如表情*/
          /*taskId:470 自如人工与环信对接
          *说明：环信表情多于自如，不同字符可能对应相同内容表情图片，重复的图片在表情中隐藏
          */
          ['[大笑]', '[):]'],
          ['[色]', '[({)]'],
          ['[流泪]', "[:'(]"],
      ...
      *说明：使用自如表情
      */
      if (this.options.src.indexOf('zr') + 1) {//自如表情
        this.obj.faceType[0] = '自如表情'
        this.obj.faceType[1] = 'png'
        this.obj.faceType[2] = 'png'
      }
    ...
    baseEl: function () {
      ...
      */
      ...
      for (var key in this.obj.face) {
        ...
        if (key == this.obj.faceType[0]) {//选择表情
          var mood = this.obj.face[key],
            html = ''
          for (var i = 0; i < this.options.rowNum * this.obj.maxNum_y-21; i++) {
            var srcHtml = '',
              title = '',
              mark = ''
            //taskId=470 环信与第三方对接 重复表情图不加载 共有21个重复表情
            if (i < mood.length-21) {
      ...
      //taskId=408 使用自如表情
      var $allMood = $('.FA_moodCtn[group=自如表情]').clone()
          ...
          try {
            ...
            if (fileData[i].type.indexOf('image') + 1) {//可预览
              ...
              reader.onload = function (e) {
                ...
                expectHeight = 900*this.naturalHeight/this.naturalWidth;
                ...
                ctx.drawImage(this, 0, 0, 900, expectHeight);
                ctx.save();
...
;(function ($, window, document, undefined) {
      ...
      */
      ...
      helpfulModule: {//答案满意度模块
        ...
        doSta:function(){
          ...
          $("#helpContent").off('touchmove').on('touchmove',function(e){
  ...
  Faqrobot.prototype = {
    init: function () {
      ...
      //taskId:408 分离图片上传以及拍照上传功能
      //说明：分别传输，照片和图片的id,用于区分添加的input标签类型
      ...
      if (this.options.upFileModule.open) {
        //this.upFile();//上传文件->s=uf
        this.upFile(this.options.upFileModule.triggerId);//上传文件->s=uf
      }
      if (this.options.upFileModuleCam.open) {
        this.upFile(this.options.upFileModuleCam.triggerId);//上传文件->s=uf
      }
    ...
    initBaseInfo: function () {
      ...
      This.request({
        ...
        callback: function (data) {
          ...
          // if((!data.talkMessageList)&&(!data.talkMessageList[0])){//taskId:406 闪退时不显示欢迎语以及导问 !data.talkMessageList[0]
          //     This.welcomeguideQue(data)
          // }
    ...
    flashOutDeal:function(data){
      ...
      for (var i = 0; i<data.talkMessageList.length; i++) {
          ...
          var obj={
            ...
            time:data.talkMessageList[i].ansTime||'',
    ...
    /*查看历史记录事件绑定  taskId=406 
    *说明：原本绑定事件方法执行一次后失效  调整绑定方式在historyRecord()中调用
    */ 
    recordEvent: function () {
      var This = this
      // $('#' + This.options.chatCtnId).on('click.FA', '.MN_record', function ()
      $('.MN_record').on('click.FA', function () {
        //taskId=406 正常查看历史记录时，显示"以上为历史记录"字样
        if(This.recordIndex==1){
          $(this).addClass('flashOutDeal').html('以上为历史记录').removeClass('MN_record');
        }
        This.historyRecord()
    ...
    sayHello: function (data) {
     ...
     getUserName:function(){
      var name,value; 
      var str=location.search; //取得查询字符串
          str=str.substr(1);   
    ...
    robotHtml: function (data, index, hotQue) {//hotQue代表欢迎语下面的热点问题
      ...
      if (data.webConfig) {//欢迎语结构
        ...
        if(data.helloWord){
            ...
            //自如定制 增加用户名
            var urlCode=this.getUserName();
            var username=decodeURI(urlCode.username);
    ...
    *@param word 问题内容
    *@param time 问题时间
    *@param msgType 问题类型 用于加载历史记录时使用
    * taskId=406 
    */
    ...
    /** taskId=408 自如定制页面
     * 说明：s=aq thirdUrl字段有值，则添加按钮，跳转原生页面
    */
    goMore:function(data){
        ...
        if(url!=undefined&&url!=null&&url!=''){
          ...
          if(url=='http://ZRQuestionDetailLinkToUrlTypeContract.ziroom.com'){
            ...
            if($('.MN_kfCtn:last .J_toHtList').length){
              ...
              return;
            }else if($('.MN_kfCtn:last .noList').length){
              return;
            }
            else{
              // btnName='我的合同';
           ...
           if($('.MN_kfCtn:last .J_toHtList').length){
            ...
            $('.MN_kfCtn:last').append('<div class="orderMore goMore"><a class="'+btnClass +'">'+btnName+'</a></div>');
          }
        }
      }
    },
    ...
    askGuideQue: function () {
      var This = this
      //TaskId:  408 自如页面定制 添加editHide 事件绑定 按钮‘我的订单’
      $('body').on('click.FA', '.MN_queList, .MN_gusList, .MN_gusList, .MN_relateList,.orderBtn', function () {//#.MN_guideQue的父级是必不可少的
        This.robot.guide = false// 退出引导
        This.scrollbar.options.autoBottom = true// 恢复自动滚动到底部
    ...
    /*taskId=447 合同展示框点击自动发送合同号 机器人直接回复*/
    sendComCode:function(){
      var This=this;
      $('.compact').off('click').on('click',function(event){
          var code=$(this).find('.order-num').html();
          This.$obj.$chatCtnId.append(This.customHtml(code))//添加我的话
          var obj={
            ansCon:'你想咨询这个订单的什么问题呢？',
            time:'',
            msgType:'text'
          }
          var robotAnswer=[];
    ...
    upFile: function (triggerId){
      // var This = this
      // var $file = $('<input type="file" class="FA_file" multiple="multiple">').css({
      //   'padding': 100,
      //   'opacity': 0
      // }).appendTo($('#' +triggerId))//this.options.upFileModule.triggerId
      var This = this
      /*TaskId:  708 自如页面优化 提高拍照和相册的点击灵敏度
      *说明：配合 扩大input type=file的覆盖范围
      */
        if(triggerId=='sendCamera'){
          var $file = $('<input type="file" class="FA_file" accept="image/*" capture="camera">').css({
            'padding': 100,
            'opacity': 0,
            'height': '100%',
            'position': 'absolute',
      ...
      H5_upload('../../' + This.options.interface + '?s=uf', this.options.upFileModule.maxNum, $file, this.$obj.$chatCtnId, function (ran) {
        ...
        /*注释原因：使用该事件时，按钮标签不能隐藏；不隐藏按钮，会导致输入文字时输入框被隐藏*/
    ...
    initInput: function () {
      ...
      */
        ...
        // $inputCtnId.attr({'placeholder':'没解决？问问小如吧'})
        // $tipWordId.hide()
      }
     ...
     //taskId=408 自如页面定制  点击发送按钮，发送后隐藏按钮，显示‘+’打开工具栏
    ...
    timeRequest: function () {
        ...
        resetTimer()
        /*taskId：418 自如自定制 会话超时推送满意度评价窗口*/
        function resetTimer () {
            clearInterval(timer);
            timer = setInterval(function () {
    ...
    preview: function () {
      ...
      if (This.options.noView != 'all') {
        This.$obj.$chatCtnId.off('click.FA').on('click.FA', 'img', function (e) {
    ...
    closeWeb: function () {
      ...
      $closeBtnId.on('click.FA', function () {
        ...
        /*taskId：418 自如定制满意度评价*/
        if(sessionStorage.getItem('dialogueUselessReasonState') == 1){
          if (sessionStorage.getItem('isShow')==0) {
            //点击关闭页面时，给评价框关闭按钮增加.closeL 记录关闭页面时已发送弹框
            $('#chatCtn').append(This.defaults.staHtml);
            sessionStorage.getItem('isShow',1);
            This.defaults.helpfulModule.doSat();
            $('.helpCtn .helpfull-close').addClass('closeL');
          } else {
            if (confirm('确定要退出吗？')) {
              This.closeWebPage();
            }
