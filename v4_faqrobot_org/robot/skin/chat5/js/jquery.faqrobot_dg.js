...
$.fn.extend({
		...
		if( (typeof url == "string") && (url.length > 0) ){
			...
			$.ajax({
				...
				success: function(data) {
					...
					if(parsed.nowState==1 && parsed.status==101){
						showErrorMsg(parsed.message);
					}
	...
	function sayHello(data){
	 ...
	 //流程问题的查看
	 $(document).on('click','.wflink',function(){
	 	$this = $(this);
	...
	//接入人工
	 $(document).on('click','.faqevent,#changepeople',function(){
    ...
    function init_leadQuestions(data){
        ...
        //未回复问题
        if(options.chatContDiv && options.chatContDiv.length>2 && data.leaveQue && data.leaveQue.length>1 ){
	...
	//截屏
		...
		captureObj.FinishedCallback = function(type, x, y, width, height, info, content, localpath) {//截屏完毕
          ...
          success: function(data) {
            if(data.status==0) {
              //$('#left_content').append(data.sendUrlMsg);
				  showMyWordsToForm(data.sendUrlMsg);
...
$.FaqRobot.defaults = {
	...
	maxTopQuestions:10,					//最多展示的常见问题的个数
