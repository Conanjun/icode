...
$.fn.extend({
            ...
            $(this).parent().addClass("robot_inputon");
                ...
                event.preventDefault();
			  	...
			  	if (/[\u4E00-\u9FA5]/i.test($(this).val())) {
					$input.val($(this).val().replace(/[\u4E00-\u9FA5]/g,''));
					showErrorDivSed('Sorry, I’m from USA. Please type English language and related ZK product and software issue. Thanks.');
					return;
				}
		...
		if( (typeof url == "string") && (url.length > 0) ){
			...
			$.ajax({
				...
				success: function(data) {
					...
					if(parsed.status==-1){
						if(parsed.message.contains('刷新')){
							showErrorMsg("Please refresh the page.");
						}else{
							options.showErrorMsg &&options.showErrorMsg(data) || showErrorMsg(parsed.message || "Your information authentication failed, please certification again.");
						}
						return;
					}
					if(parsed.status==-2){//身份认证错误，需要重新登录
                        options.showErrorMsg&&options.showErrorMsg(data) || showErrorMsg(parsed.message || "Your information authentication failed, please certification again.",options.authUrl);
						return;
					}
				...
				error:function() {
					var parsed = options.showErrorMsg && options.showErrorMsg(data) || showErrorMsg("Data request error, please refresh!");
				}
			});
		} 
	};
	...
	function sayHello(data){
        if(data.status==-1){
            showErrorDiv("Network is not to force, please try refreshing the page.");
            return;
        }
        if(data.status==-2){//身份认证错误，需要重新登录
            showErrorDiv("Your information authentication failed, please certification again.");
		...
		if(typeof data.webConfig.logoUrl!="undefined"){
			options.logoUrl=data.webConfig.logoUrl==''?"skin/chat55/images/logo.png":data.webConfig.logoUrl;
    ...
    function init_leadQuestions(data){
		...
		var all_logo=$('<p/>').addClass('i').html('<img src="skin/chat55/images/fi1.png"  width="50">').appendTo(allObj);
		...
		if(options.baseInfoDiv && options.baseInfoDiv.length>2){
			if(typeof data.webConfig!="undefined"){
			  ...
			  if(typeof data.webConfig.webName!="undefined"){
					cardhtml2+='<dt>Company Name:&nbsp;&nbsp;'+data.webConfig.webName+'</dt>';
			  }
			  if(typeof data.webConfig.serviceTel!="undefined"){
				    cardhtml2+='<dt>Contact number:&nbsp;&nbsp;'+data.webConfig.serviceTel+'</dt>';
			  }
			  if(typeof data.webConfig.webSite!="undefined"){
					cardhtml2+='<dt>Company website:&nbsp;&nbsp;<a href="'+data.webConfig.webSite+'" target="_black">'+data.webConfig.webSite+'</a></dt>';
			  }
			}
    ...
    function showErrorDiv(msg,fun){
		...
		//出现错误提示框
    function showErrorDivSed(msg){
        showerrorDiv.find('span').html(msg);
        showerrorDiv.find('span').delay(1000).fadeTo("slow", 0.2).fadeTo("slow", 1).fadeOut(4000,function(){
			showerrorDiv.find('span').html('');
		}).stop();
    }
	/*********************下面是机器人网络交互的部分*************************/
    ...
    function createMyWordsHtml(msg,time){
		var myword='<div class="msgright"><p class="ri"><img width="55" src="skin/chat55/images/fi2.jpg" style="margin-left:5px;"></p><div class="rmsgcontent"><p class="spacing15"></p><div class="metext"><span class="s1">Me：</span><span>'+msg+'</span>&nbsp;&nbsp;<br><span class="s2">'+time+'</span><p class="spacing5"></p></div><div class="ishaveyong"><p class="spacing5"></p></div><p class="spacing"></p></div></div><p class="spacing15"></p>';
        return myword;
    }
	...
	function createRobotWordsHtmlItem(ansItem,time){
		...
		if(typeof ansItem.gusWords !='undefined' && ansItem.gusWords!=null ){
			beforeWords=ansItem.gusWords.ydWords==''?'Did you want to consult the following problems?':ansItem.gusWords.ydWords;
			afterWords=ansItem.gusWords.afterWords==''?'':ansItem.gusWords.afterWords;
		}else{
			beforeWords='Did you want to consult the following problems?';
			afterWords='';
		}
		if(!ansItem){
			return '';
        ...
        if( typeof ansItem.aId!="undefined" && ansItem.aId!=0){
            ...
            $('<a/>').addClass("robot_review_yes").attr("helptag",1).attr("aid",aid).click(helpclick).html('<span></span>like').appendTo(helpDiv);
            $('<a/>').addClass("robot_review_no").attr("helptag",0).attr("aid",aid).click(helpclick).html('<code></code>tread').appendTo(helpDiv);
        }else{
        }
        //相关问题(右侧应该配置div的ID)
        if( typeof ansItem.aId!="undefined" && ansItem.aId!=0){
            if(options.sugQuestionDiv && options.sugQuestionDiv.length>2 && typeof ansItem.relateLessList!="undefined" && ansItem.relateLessList.length>1 ){
                ...
                var  lielement = $("<dt/>").html("Did you want to consult the following problems?").appendTo(element).css('color','red');
		...
		if(ansItem.ansCon!="" && ansItem.ansCon.length>0){
            var listAll = '';
			var nscon=ansItem.ansCon;
			var sear=new RegExp('正在等待人工接入');
		　  if(sear.test(nscon))
		　 　{
				nscon='Please wait while  customer service staff is accessing';
			}
            all_robot_info.html(nscon);
        }
    ...
    function offLine(){
       var s_q_url=options.basePath+'AQ?s=offline&nocache='+new Date().getTime();
	    $.getJSON(
            ...
            function(data){
				if(data.status==0){
					var obj=eval('({"robotAnswer":{"ansCon":"You have been offline, thank you for your use! If you need to chat again, please open or refresh this page again.","status":0,"tspan":2}})');
   ...
   //字数统计
    ...
    function helpclick(){
            ...
            $('.helper_aid_'+aid).text('Thank you for rating!').css('color','#666');
            var s_q_url='';
            if(usefull==1){
	...
	function Evaluation(){
		if($('.feedback').length>0){
			OAnswer.push('<div class="msgleft"><p class="i"><img width="50" src="skin/chat55/images/fi1.png"></p><div class="msgcontent"><p class="spacing"></p><div class="kftext"><div><span class="s1">'+options.robotName+'</span>：<p>感谢您的支持，您已经做过评价了！</p><span class="s2">2015-4-7 10:58:40</span></div><p class="spacing5"></p><div class="ishaveyong"><p class="spacing5"></p><span></span></div></div><p class="spacing"></p></div></div><p class="spacing15"></p>');
		}else{
