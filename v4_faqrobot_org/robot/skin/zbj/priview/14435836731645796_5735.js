...
var body = document.getElementsByTagName('body')[0];var div = document.createElement('div');body.appendChild(div);div.outerHTML='<div class="qqchat qqchat_5735"><div class="showchat" style="display: none;"><div class="header"><h2>常见问题</h2></div><ul class="content"></ul><span class="close">&times;</span></div><div class="hidechat"><img class="yunPig" src="http://v3.faqrobot.org/robot/skin/zbj/private/chat-icon.png" /></div></div>';
hasJquery();
setTimeout(function(){
	//操作事件
	$(".qqchat_5735").on("mouseover mouseout", function(e) {
		if(e.type == "mouseover") {
			$(".qqchat_5735 .showchat").show();
		}else {
			$(".qqchat_5735 .showchat").hide();
		}
	});
	$(".qqchat_5735 .close").on("click", function() {
		$(".qqchat_5735 .showchat").hide();
	});
	$('.yunPig').click(function(){
		openWin('http://v3.faqrobot.org/robot/zbj.html?jid=5735','870','600');
	});
	//点击其他问题
...
function showChatModal(obj){
	openWin('http://v3.faqrobot.org/robot/zbj.html?jid=5735&que='+$(obj).html(),'870','600');
}
//显示常见问题
function commonQue(){
	$.ajax({
		...
		datatype:'json',
		cache:false,
		url:'http://v3.faqrobot.org/servlet/AQ?s=basep&sysNum=14435836731645796&jid=5735',
		dataType:'jsonp',
		callback:'callback',
		success:
		function(data){
			if(data.newAdd && data.newAdd.length>0){
				...
				$('.qqchat_5735 .content').html(temp.join(''));
			}
		}
	})
}
