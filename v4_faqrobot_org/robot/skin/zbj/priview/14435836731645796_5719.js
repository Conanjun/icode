...
var body = document.getElementsByTagName('body')[0];var div = document.createElement('div');body.appendChild(div);div.outerHTML='<div class="qqchat qqchat_5719"><div class="showchat" style="display: none;"><div class="header"><h2>常见问题</h2></div><ul class="content"></ul><span class="close">&times;</span></div><div class="hidechat"><img class="yunPig" src="http://v3.faqrobot.org/robot/skin/zbj/private/chat-icon.png" /></div></div>';
hasJquery();
setTimeout(function(){
	//操作事件
	$(".qqchat_5719").on("mouseover mouseout", function(e) {
		if(e.type == "mouseover") {
			$(".qqchat_5719 .showchat").show();
		}else {
			$(".qqchat_5719 .showchat").hide();
		}
	});
	$(".qqchat_5719 .close").on("click", function() {
		$(".qqchat_5719 .showchat").hide();
	});
	$('.yunPig').click(function(){
		openWin('http://v3.faqrobot.org/robot/zbj.html?jid=5719','870','600');
	});
	//点击其他问题
...
function showChatModal(obj){
	openWin('http://v3.faqrobot.org/robot/zbj.html?jid=5719&que='+$(obj).html(),'870','600');
}
//显示常见问题
function commonQue(){
	$.ajax({
		...
		datatype:'json',
		cache:false,
		url:'http://v3.faqrobot.org/servlet/AQ?s=basep&sysNum=14435836731645796&jid=5719',
		dataType:'jsonp',
		callback:'callback',
		success:
		function(data){
			if(data.newAdd && data.newAdd.length>0){
				...
				$('.qqchat_5719 .content').html(temp.join(''));
			}
		}
	})
}
