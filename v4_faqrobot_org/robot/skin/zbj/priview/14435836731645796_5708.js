...
var body = document.getElementsByTagName('body')[0];var div = document.createElement('div');body.appendChild(div);div.outerHTML='<div class="qqchat qqchat_5708"><div class="showchat" style="display: none;"><div class="header"><h2>常见问题</h2></div><ul class="content"></ul><span class="close">&times;</span></div><div class="hidechat"><img class="yunPig" src="http://v3.faqrobot.org/robot/skin/zbj/private/chat-icon.png" /></div></div>';
hasJquery();
setTimeout(function(){
	//操作事件
	$(".qqchat_5708").on("mouseover mouseout", function(e) {
		if(e.type == "mouseover") {
			$(".qqchat_5708 .showchat").show();
		}else {
			$(".qqchat_5708 .showchat").hide();
		}
	});
	$(".qqchat_5708 .close").on("click", function() {
		$(".qqchat_5708 .showchat").hide();
	});
	...
	$.ajax({
		...
		function(data){
			if(data.newAdd && data.newAdd.length>0){
				...
				$('.qqchat_5708 .content').html(temp.join(''));
			}
		}
	})
}
