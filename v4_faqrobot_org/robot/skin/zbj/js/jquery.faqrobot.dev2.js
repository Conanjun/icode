	...
	function createRobotWordsHtmlItem(ansItem,time){
        ...
        if( typeof ansItem.aId!="undefined" && ansItem.aId!=0){
            ...
            rightCon_helpWords.addClass("helper_aid_"+aid);
			 $('<a/>').css('color','#333').css('font-size','14px').html('以上内容对您有帮助吗？').appendTo(rightCon_helpWords);
            $('<a/>').addClass('helpA').attr("helptag",1).attr("aid",aid).click(helpclick).html('<img src="skin/zbj/images/good.png">有&nbsp;').appendTo(rightCon_helpWords);
            $('<a/>').addClass('helpA').attr("helptag",0).attr("aid",aid).click(helpclick).html('<img src="skin/zbj/images/bad.png">没有').appendTo(rightCon_helpWords);
