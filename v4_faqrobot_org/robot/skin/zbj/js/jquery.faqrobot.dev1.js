...
$.fn.extend({
	...
	function sayHello(data){
		...
		if(typeof data.webConfig.logoUrl!="undefined"){
			options.logoUrl=data.webConfig.logoUrl==''?"skin/zbj/image/logo.png":data.webConfig.logoUrl;
...
/*	function init_webLogo(data){
	...
	function createRobotWordsHtmlItem(ansItem,time){
        ...
        if( typeof ansItem.aId!="undefined" && ansItem.aId!=0){
            ...
            $('<a/>').addClass("robot_review_yes").attr("helptag",1).attr("aid",aid).click(helpclick).html('<span></span>满意').appendTo(rightCon_helpWords);
            $('<a/>').addClass("robot_review_no").attr("helptag",0).attr("aid",aid).click(helpclick).html('<code></code>不满意').appendTo(rightCon_helpWords);
...
$.FaqRobot.defaults = {
	...
	logoUrl:"skin/zbj/image/logo.png",
	...
	kfUrl:'skin/zbj/image/logo.png',  //客服图标
	khUrl:'skin/zbj/image/robot_askicon.png', //客户图标
