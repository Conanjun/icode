...
$.fn.extend({
	faqrobot: function(sysNum, options) {
		...
		options = $.extend({}, $.FaqRobot.defaults, {
            //默认配置选项
			sysNum: isSysNum ? sysNum : 1000000
		}, options);
		...
		if( (typeof url == "string") && (url.length > 0) ){
			...
			$.ajax({
				...
				success: function(data) {
					...
					//滚动到底部
					chatContDIv.parent().scrollTop(100000);
	...
	function sayHello(data){
		...
		if(typeof data.webConfig.logoUrl!="undefined"){
			options.logoUrl=data.webConfig.logoUrl==''?"skin/zbj/images/logo.png":data.webConfig.logoUrl;
			$('#'+options.logoDiv).html('<img src="'+options.logoUrl+'">');
		}
		//设置机器人聊天图标
		...
		if(typeof data.webConfig.level!="undefined" &&  data.webConfig.level<3){
			if(urlId[0].indexOf('fees')>0){
				showErrorDiv('您不是付费用户，即将跳转到免费页面',function(){
					location.href="faqrobot.html?sysNum="+options.sysNum;
				})
			}
		}else{
			$(document).attr("title",data.webConfig.webName);
			$('#'+options.advFaqrobot).hide();
		}
	...
	function init_recommend(recommendata){
		if(options.recommend && options.recommend.length>2 && typeof recommendata!="undefined" && recommendata.length>0 ){
			var html=[];
			for(var i=0;i<recommendata.length;i++){
				if(recommendata[i].type==0){
					html.push('<dt><a href="'+recommendata[i].linkurl+'" target="_blank">'+(i+1)+'.'+recommendata[i].content+'</a></dt>');
				}else{
					html.push('<dt><a href="'+recommendata[i].linkurl+'" target="_blank">'+(i+1)+'.<img src="'+recommendata[i].content+'"></a></dt>');
				}
			}
    ...
    function init_leadQuestions(data){
        var allObj = $('<div/>').addClass("robot_question").html('<div class="robot_questionR_time">猪八戒智能客服&nbsp;'+getTime()+'</div>');
    ...
    function formatQueULList(targetId,list,ulclass,maxCount,maxWords){
        ...
        for (var i=0; i < max; i++) {
            ...
            if ( formattedCont === false )
                continue;
            var childElement= $("<a/>");
            childElement.html((i+1)+'.'+formattedCont)
            .appendTo(lielement)
    ...
    function createMyWordsHtml(msg,time){
        var myword='<div class="robot_ask"><dl class="robot_askL"><dd>我&nbsp;'+time+'</dd><dt>'+msg+'<span></span></dt></dl></div>';
        return myword;
    }
	...
	function createRobotWordsHtmlItem(ansItem,time){
        ...
        if( typeof ansItem.aId!="undefined" && ansItem.aId!=0){
            ...
            rightCon_helpWords.addClass("helper_aid_"+aid);
			 $('<a/>').addClass('tipbtn').html('以上内容对您有帮助吗？').appendTo(rightCon_helpWords);
            $('<a/>').addClass('helpA helpbtn').attr("helptag",1).attr("aid",aid).click(helpclick).html('有&nbsp;').appendTo(rightCon_helpWords);
            $('<a/>').addClass('helpA lessbtn').attr("helptag",0).attr("aid",aid).click(helpclick).html('没有').appendTo(rightCon_helpWords);
        ...
        if( typeof ansItem.aId!="undefined" && ansItem.aId!=0){
            if(options.sugQuestionDiv && options.sugQuestionDiv.length>2 && typeof ansItem.relateLessList!="undefined" && ansItem.relateLessList.length>1 ){
                var rightCon_relate=$('<div/>').addClass("questionR_txt").appendTo(a_rightCon);
   ...
   //字数统计
	...
	//滚动条
	function yun_scroll(){
		$(".nano").nanoScroller({scrollTo: $('.conDiv>div:last')});
		$(".nano").nanoScroller({
			alwaysVisible: true
		});
	}
...
$.FaqRobot.defaults = {
	...
	logoUrl:"skin/zbj/images/logo.png",
	...
	maxTopQuestions:5,					//最多展示的常见问题的个数
	...
	kfUrl:'skin/zbj/images/logo.png',  //客服图标
	khUrl:'skin/fees1/image/robot_askicon.png', //客户图标
