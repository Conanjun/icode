...
$.fn.extend({
	...
	var getjid=[];
	if(comeUrl[1] && comeUrl[1].indexOf("jid=")!==-1){
		...
		if( (typeof url == "string") && (url.length > 0) ){
			...
			$.ajax({
				...
				success: function(data) {
					...
					if(parsed.status==0 && parsed.tspan=='2000'){
						showErrorMsg(parsed.message || "您长时间未交互，请刷新页面");
						return;
					}
	...
	function initRobotBaseInfo(){
		...
		//options.basePath
		var infoUrl = options.basePath +'AQ?s=p&sysNum='+options.sysNum+'&sourceId='+options.sourceId+'&'+getjid[0];
	...
	function sayHello(data){
		...
		if(typeof data.webConfig.logoUrl!="undefined"){
			options.logoUrl=data.webConfig.logoUrl==''?"skin/anran/images/head_chat.png":data.webConfig.logoUrl;
	...
	//接入人工
	 $('.faqevent').live('click',function() {
         ...
         var hrefUrl=options.basePath+'AQ?'+hrefValue+'&sysNum='+options.sysNum+'&sourceId='+options.sourceId+'&'+getjid[0];
    ...
    function init_leadQuestions(data){
        ...
        //未回复问题
        if(options.chatContDiv && options.chatContDiv.length>2 && typeof data.leaveQue!="undefined" &&data.leaveQue!=null&& data.leaveQue.length>1 ){
         //   helo+='<div class="f_deft"><p>您好，我是机器人<span class="nameTag">'+rname+'</span>，这里有您的留言，查看留言请点击<a href="javascript:void(0)" id="showLeaveMsg"> 这里.</a></p></div>';
           /* rightCon_robotWords.html('<p>您好，我是机器人<span class="nameTag">'+data.webConfig.robotName+'</span>,这里有您的留言，查看留言请点击</p>');
            ...
            robot_icon_small.html(data.helloWord || data.webConfig.helloWord);
	...
	function askQue(msg,slutionId){
        if(msg && msg.length>0){
            var askUrl = options.basePath +'AQ?s=aq&sysNum='+options.sysNum+'&sourceId='+options.sourceId+'&'+getjid[0];
            if(slutionId && slutionId++>0){
                askUrl+='&sId='+slutionId;
            }
            //首先把自己的问题内容显示到聊天框里面去
	...
	function createRobotWordsHtmlItem(ansItem,time){
		if(!ansItem.ansCon && ansItem.relateList.length==0 && ansItem.gusList.length==0){return;}
		var beforeWords='';
		var afterWords='';
		//定义引导问题前后的提示语
        ...
        if( typeof ansItem.aId!="undefined" && ansItem.aId!=0){
            ...
            $('<a/>').addClass("robot_review_no").attr("helptag",0).attr("aid",aid).attr("cluid",cluid).click(helpclick).html('<span class="robot_help_no"></span>').appendTo(robot_help);
            $('<a/>').addClass("robot_review_yes").attr("helptag",1).attr("aid",aid).attr("cluid",cluid).click(helpclick).html('<span class="robot_help_yes"></span>').appendTo(robot_help);
		...
		if(ansItem.ansCon!="" && ansItem.ansCon.length>0){
			...
			robot_icon_small.html(replace_em(curCon));
		}
		//推荐问题
	...
	function replace_em(str){
		str = str.replace(/\[微笑\]/g,'<img src="/onlineMonitor/monitor/src/yun/0.png" border="0" />');
		str = str.replace(/\[色\]/g,'<img src="/onlineMonitor/monitor/src/yun/1.png" border="0" />');
		str = str.replace(/\[得意\]/g,'<img src="/onlineMonitor/monitor/src/yun/2.png" border="0" />');
		str = str.replace(/\[流泪\]/g,'<img src="/onlineMonitor/monitor/src/yun/3.png" border="0" />');
		str = str.replace(/\[害羞\]/g,'<img src="/onlineMonitor/monitor/src/yun/4.png" border="0" />');
		str = str.replace(/\[闭嘴\]/g,'<img src="/onlineMonitor/monitor/src/yun/5.png" border="0" />');
		str = str.replace(/\[发怒\]/g,'<img src="/onlineMonitor/monitor/src/yun/6.png" border="0" />');
		str = str.replace(/\[呲牙\]/g,'<img src="/onlineMonitor/monitor/src/yun/7.png" border="0" />');
		str = str.replace(/\[惊讶\]/g,'<img src="/onlineMonitor/monitor/src/yun/8.png" border="0" />');
		str = str.replace(/\[难过\]/g,'<img src="/onlineMonitor/monitor/src/yun/9.png" border="0" />');
		str = str.replace(/\[酷\]/g,'<img src="/onlineMonitor/monitor/src/yun/10.png" border="0" />');
		str = str.replace(/\[愉快\]/g,'<img src="/onlineMonitor/monitor/src/yun/11.png" border="0" />');
		str = str.replace(/\[流汗\]/g,'<img src="/onlineMonitor/monitor/src/yun/12.png" border="0" />');
		str = str.replace(/\[奋斗\]/g,'<img src="/onlineMonitor/monitor/src/yun/13.png" border="0" />');
		str = str.replace(/\[疑问\]/g,'<img src="/onlineMonitor/monitor/src/yun/14.png" border="0" />');
		str = str.replace(/\[晕\]/g,'<img src="/onlineMonitor/monitor/src/yun/15.png" border="0" />');
		str = str.replace(/\[委屈\]/g,'<img src="/onlineMonitor/monitor/src/yun/16.png" border="0" />');
		return str;
	}
	
    //下线的事件
    function offLine(){
       var s_q_url=options.basePath+'AQ?s=offline&sysNum='+options.sysNum+'&sourceId='+options.sourceId+'&'+getjid[0]+'&nocache='+new Date().getTime();
	    $.getJSON(
            s_q_url,
            function(data){
				if(data.status==0){
	...
	function autoRequest(){
		...
		var autoR_url=options.basePath+'AQ?s=kl&sysNum='+options.sysNum+'&sourceId='+options.sourceId+'&'+getjid[0];
		$.ajax({
			type:'post',
			datatype:'json',
			cache:false,
			url:options.basePath+'AQ?s=kl&sysNum='+options.sysNum+'&sourceId='+options.sourceId+'&'+getjid[0],
   ...
   //字数统计
    ...
    function helpclick(){
            ...
            var cluid=$this.attr("cluid");
            var usefull=$this.attr("helptag");
            $('.helper_aid_'+aid).html('感谢您的评价！');
            var s_q_url='';
            if(usefull==1){
                s_q_url=options.basePath+'AQ?s=addufc&aId='+aid+'&sysNum='+options.sysNum+'&cluid='+cluid+'&sourceId='+options.sourceId+'&'+getjid[0];
            }else{
                s_q_url=options.basePath+'AQ?s=addulc&aId='+aid+'&sysNum='+options.sysNum+'&cluid='+cluid+'&sourceId='+options.sourceId+'&'+getjid[0];
            }
            request(s_q_url,function(data){
                $('.helper_aid_'+aid).html(data.message);
            });
        }
    //****************************************公用方法******************************************//
    ...
    function fadeback(){
        ...
        var fadeUrl=options.basePath+'AQ?s=fadeback&sysNum='+options.sysNum+'&sourceId='+options.sourceId+'&'+getjid[0];
        var val2="";
	...
	//点击引导语
	$('#'+options.chatContDiv).on('click','.welcomeWords',function(){
		askQue($(this).html());
	})
...
$.FaqRobot.defaults = {
	...
	kfUrl:'skin/anran/images/head_chat.png',  //客服图标
	khUrl:'skin/anran/images/peo4.png', //客户图标
