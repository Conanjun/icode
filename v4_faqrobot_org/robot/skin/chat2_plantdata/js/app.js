;$(function() {
   ...
   if(getUrlParam('lan') && getUrlParam('lan')=='en'){
       //英文模式
       FAQ = new Faqrobot({
           logoUrl: 'robot/skin/chat2_plantdata/images/PlantRobot.png',//logo地址 ----------
           logoId: 'logo',
           ...
           webName:'KGRobot',
            kfHtml: [
                   ...
                   '<div class="MN_helpful"><span class="MN_yes"><i class="fa fa-thumbs-o-up"></i>&nbsp;有用</span><span class="MN_no"></span><span class="line"></span><i class="fa fa-thumbs-o-down"></i>&nbsp;没用</span></div>',//taskId=823 是否满意组合
            ...
            * 修改：添加服图标分为机器人和人工客服
            */
           kfPic: 'robot/skin/chat2_plantdata/images/robot_plantdata.png',  //客服图标
           kf_Robot_Pic: 'robot/skin/chat2_plantdata/images/robot_plantdata.png',  //机器人客服图标 taskId=823 plantdata定制  Amend by zhaoyuxing
           kf_Person_Pic: 'robot/skin/chat2_plantdata/images/robot_plantdata.png',  //人工客服图标
           kf_Robot_Name:'',//机器人客服名字，此处只是声明个变量，不用赋值
           kf_Person_Name:'',//人工客服名字

           khPic: 'robot/skin/chat2_plantdata/images/plantKh.png', //客户图标
           //taskId=823 plantdata定制时间格式 Amend by zhaoyuxing
           formatDate: '%hour%:%minute%:%second% %month%-%date%',//配置时间格式(默认10:42:52 2016-06-24)
       ...
       FAQ = new Faqrobot({
            ...
            */
           kfPic: 'robot/skin/chat2_plantdata/images/robot_plantdata.png',  //客服图标
           kf_Robot_Pic: 'robot/skin/chat2_plantdata/images/robot_plantdata.png',  //机器人客服图标
           kf_Person_Pic: 'robot/skin/chat2_plantdata/images/robot_plantdata.png',  //人工客服图标
           kf_Robot_Name:'',//机器人客服名字，此处只是声明个变量，不用赋值
           kf_Person_Name:'',//人工客服名字
           ...
           khPic: 'robot/skin/chat2_plantdata/images/plantKh.png', //客户图标
           //taskId=823 plantdata定制时间格式 Amend by zhaoyuxing
           formatDate: '%month%-%date% %hour%:%minute%:%second%',//配置时间格式(默认10:42:52 2016-06-24)  
        // topQueId: 'commonQue',//热门、常见问题Id --------
           quickServId: 'quickLink',//快捷服务Id
           thirdUrlId:'thirdUrl',
           chatCtnId: 'chatCtn',//聊天展示Id y   --------------
           ...
           thirdUrlCallBack:function(data,index){
               ...
               if(data.robotAnswer[index].thirdUrl && data.robotAnswer[index].thirdUrl.url){
                   $('.itemHead5').trigger('click');
                   $('.itemHead1').addClass('itemHeadFocus').text('推荐链接');
                   if(!$('.artiSearch').hasClass('artiSearchHide')){
                       $('.artiSearch').addClass('artiSearchHide')
                   }
                   $('#'+ FAQ.options.thirdUrlId+' iframe').attr('src',data.robotAnswer[index].thirdUrl.url);
                }else{
                   $('.itemHead1').addClass('itemHeadFocus').text('热门技能');
                   $('.itemHead1').trigger('click');
                };
           },
       });
   ...
   */
   ...
   $('body').on('click', '.itemHead', function() {
       ...
       /*
            taskid=217，黄世鹏
            修改：plantdata页面嵌入iframe
        */
        if($(this).hasClass('itemHead1')){
            $('#outIframe').show()
        }else{
            $('#outIframe').hide()
        }
        /*
            taskid=353,黄世鹏
            修改：热门技能和推荐链接共享一个tab展示区
        */ 
        if($(this).hasClass('itemHead1') && $(this).text()=='推荐链接'){
           $('.itemHead5').trigger('click');
           $('.itemHead1').addClass('itemHeadFocus').text('推荐链接');
        }
   });
   ...
   $(window).on('resize.TH', function() {
       /*
           ...
           开发：添加右侧打开收起按钮
       */ 
       if(!$('#hideArrow').hasClass('openArrow')){
           set_whole_size();
           set_body_height();
           set_bodyLeftTop_height();
           set_bodyLeft_width();
           ...
           set_bodyLeft_width();
           set_bodyright_iframe_height();
           $('.bodyLeft').width($('.whole').outerWidth());
       }
   });
   ...
   */ 
   $('body').on('click','#hideBtn',function(){
       if($('#hideArrow').hasClass('openArrow')){
           ...
           set_bodyLeft_width();
           set_bodyright_iframe_height();
           $('#hideArrow').removeClass('openArrow')
           $('.bodyRight').show();
           $('#chatCtn').append('<div class="MN_kfCtn" style="display:none"> </div>')
       }else{
           $('.bodyRight').hide();
           $('.bodyLeft').width($('.whole').outerWidth());
           $('#hideArrow').addClass('openArrow')
           $('#chatCtn').append('<div class="MN_kfCtn" style="display:none"> </div>')
       }
...
/**
   ...
   */
   ...
   // taskId=823 鼠标移入时更改图片
   $('#chatCtn').on('mouseenter','.MN_answer .MN_helpful .MN_yes',function(){
       var _icon=$(this).find('.usefull');
       _icon.addClass('hover');
   })
   $('#chatCtn').on('mouseleave','.MN_answer .MN_helpful .MN_yes',function(){
       var _icon=$(this).find('.usefull');
       _icon.removeClass('hover');
   })
   $('#chatCtn').on('mouseenter','.MN_answer .MN_helpful .MN_no',function(){
       var _icon=$(this).find('.useless');
       _icon.addClass('hover');
   })
   $('#chatCtn').on('mouseleave','.MN_answer .MN_helpful .MN_no',function(){
       var _icon=$(this).find('.useless');
       _icon.removeClass('hover');
   })
   $("#outIframe").css("height",($(".bodyRight").css("height").split("px")[0]-$(".itemBtnCtn").css("height").split("px")[0]))
});