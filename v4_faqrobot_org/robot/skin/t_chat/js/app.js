;$(function() {
    ...
    var FAQ = new Faqrobot({
        ...
        getCallback: function(answer, data) {//获取到答案后的回调
            //判断数据的链接地址url是否存在
            if(data.robotAnswer[0].thirdUrl.url !==''){
                $('.artiSearch').removeClass('artiSearchHide');
                $('.itemCtn').css('width', '25%');
                $('.itemHead4').trigger('click');
                $('iframe').attr('src', data.robotAnswer[0].thirdUrl.url);
                var CurMN_queList=$('.MN_queList').find('span:eq(1)').text();
                $('.itemHead4').html(CurMN_queList);
            }
            else{
                $('.artiSearch').addClass('artiSearchHide');
                $('.itemCtn').removeAttr('style');
    ...
    function set_iframeCtx_height() {
        ...
        $('.iframeCtx').height($('.bodyRightCtx').height() - $('.itemBtnCtn').height());
    }
