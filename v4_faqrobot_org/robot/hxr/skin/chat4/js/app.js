...
$(function() {
	...
	var FAQ = new Faqrobot({
	    ...
	    logoUrl: 'robot/hxr/skin/base2/images/logomiddle.png',//logo地址 ----------
	    logoId: 'weblogo',// ----------
	    webNameId: 'title',//公司名称Id
	    //userInfoId: 'userInfoId',//用户信息Id
    	//webInfoId: 'webInfoId',//公司简介Id
	    kfPic: 'robot/hxr/skin/chat4/images/robot.png',  //客服图标
	    khPic: 'robot/hxr/skin/chat4/images/user.png', //客户图标
        ...
        initCallback: function(data) {//初始化基本信息的回调
			$('.rightBtn3').on('click', function() {
				if(!window.uselessReasonItems) {
					$('.contact').empty().append('<div style="text-align: center; height: 150px;"><div style="height:150px;display:table-cell;vertical-align:middle;width: 260px;"><img class="contactImg" style="max-height: 150px;" src="'+ (data.webConfig.logoUrl || '') +'"></div></div><p class="contact1">'+ (data.webConfig.robotName || '') +'</p><a href="'+ (data.webConfig.webSite || '') +'" target="_blank"><p class="contact2">'+ (data.webConfig.webSite || '') +'</p></a>');
