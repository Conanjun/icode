...
$.fn.extend({
	...
	function sayHello(data){
		...
		if(typeof data.webConfig.level!="undefined" &&  data.webConfig.level>=3){
			$('#devP').hide();
			$('.link_a').hide();
			$('#'+options.logoDiv).parent('a').removeAttr('title');
		}
        ...
        $("#"+options.clearScreen).click(function(){
			...
			init_leadQuestions(data);
			//^^^^^^^^^^更新滚动条
			$("#"+options.chatContDiv).parent().scrollTop(0);
			setScroll();
	...
	$('#'+options.chatContDiv).on('click','.welcomeWords',function(){
	 ...
	 //流程问题的查看
    $('#left_content').on('click','.wflink',function() {
        $this = $(this);
	...
	//接入人工
	 $('#left_content').on('click','.faqevent',function(event) {
		 event.preventDefault();  
	...
	function init_quickLink(quickLink){
		if(options.quickLink && options.quickLink.length>2 && typeof quickLink!="undefined" && quickLink.length>0 ){
			...
			for(var i=0;i<quickLink.length;i++){
				html.push('<a href="'+quickLink[i].linkUrl+'" target="_blank"><img src="'+quickLink[i].imageUrl+'"><br><span>'+(!quickLink[i].name?'&nbsp;':quickLink[i].name)+'</span></a>')
			}
...
/*	function init_webLogo(data){
    ...
    function init_leadQuestions(data){
		...
		var allObj = '<div class="serv chat-ctn"><span class="serv-icon"><img src="'+ options.kfUrl +'" /></span><div class="serv-ctn wc-pre"><i class="triangle triangle1"></i><i class="triangle triangle2"></i><p class="say-word pre-say-word">'+data.webConfig.helloWord+'</p><span class="nameWrap"><span class="r2-name">'+options.robotName+'</span><i class="time">'+getTime()+'</i></div></div>';
	...
	function init_baseInfo(data){
		...
		if(options.baseInfoDiv && options.baseInfoDiv.length>2){
			if(options.baseInfoCustom) {
				if(typeof data.company!="undefined" && data.company){
				  cardhtml2='';
				  if(typeof data.webConfig.logoUrl!="undefined"){
					  if(data.webConfig.logoUrl!==""){
						$('.'+options.baseInfoDiv).css('background', '#fff no-repeat center top');
						cardhtml2+='<div style="text-align: center; height: 150px;"><div style="height:150px;display:table-cell;vertical-align:middle;width: 260px;"><img class="contactImg" style="max-height: 150px;" src="'+data.webConfig.logoUrl+'"></div></div>';
					  } else {
						 cardhtml2+='<div style="height:150px; width: 260px;"></div>';
					  }
				  } else {
					cardhtml2+='<div style="height:150px; width: 260px;"></div>';
				  }
				  if(typeof data.company.webName!="undefined"){
					  if(data.company.webName!=""){
						cardhtml2+='<p class="contact1">'+data.company.webName+'</p>';
					  }
				  } else {
					  cardhtml2+='<p class="contact1"> </p>';
				  }
				  if(typeof data.company.webSite!="undefined"){
					  if(data.company.webSite!=""){
						cardhtml2+='<a href="'+data.company.webSite+'" target="_blank"><p class="contact2">'+data.company.webSite+'</p></a>';
					  }
				  }
				  if(typeof data.company.tel!="undefined"){
					  if(data.company.tel!=""){
						cardhtml2+='<p class="contact4">'+data.company.tel+'</p>';
					  }
				  }
				  if(typeof data.company.qq!="undefined"){
					  if(data.company.qq!=""){
						cardhtml2+='<a href="http://wpa.qq.com/msgrd?v=3&uin='+data.company.qq+'&site=qq&menu=yes" target="_blank"><p class="contact2"><img src="http://www.faqrobot.org/images/Service1.png"></a>';
					  }
				  }
				  if(typeof data.company.address!="undefined"){
					  if(data.company.address!=""){
						cardhtml2+='<p>'+data.company.address+'</p>';
					  }
				  }
				  if(typeof data.company.openTime!="undefined"){
					  if(data.company.openTime!=""){
						cardhtml2+='<p>'+data.company.openTime+'</p>';
					  }
				  }
				}else{
					cardhtml2+='<div style="text-align: center; height: 150px;"><div style="height:150px;display:table-cell;vertical-align:middle;width: 260px;"></div></div><p class="contact1">云问科技</p><a target="_blank" href="http://www.faqrobot.org/"><p class="contact2">www.faqrobot.org</p></a><p class="contact3">南京市江宁区胜利路89号紫金研发创业中心3号楼</p><p class="contact4">025-52208363</p><a target="_blank" href="http://wpa.qq.com/msgrd?v=3&amp;uin=1765280298&amp;site=qq&amp;menu=yes"><p class="contact2"></p><p class="contact5">market@FaqRobot.org</p></a>';
				}
  ...
  function createMyWordsHtml(msg,time){
		var myword='<div class="customer chat-ctn"><span class="serv-icon"><img src="'+ options.khUrl +'" /></span><div class="serv-ctn"><i class="triangle triangle1"></i><i class="triangle triangle2"></i><p class="say-word">'+msg+'</p><i class="time">我&nbsp;&nbsp;'+time+'</i></div> </div><div class="cl"></div>';
        return myword;
    }
   ...
   //字数统计
    ...
    function helpclick(){
            ...
            $('.helper_aid_'+aid).text('感谢您的评价!').css('color','#222');
            var s_q_url='';
            if(usefull==1){
...
$.FaqRobot.defaults = {
	...
	kfUrl:'skin/base2/images/robot.png',  //客服图标
	khUrl:'skin/base2/images/user.png', //客户图标
	//>>>>>>>>>>>>>>
	quickLink:'quickLink',
