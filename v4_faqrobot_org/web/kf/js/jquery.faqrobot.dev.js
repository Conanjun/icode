...
$.fn.extend({
	...
	var getjid=[];
	if(typeof comeUrl[1]!='undefined' && comeUrl[1].indexOf("jid=")!==-1){
		...
		if( (typeof url == "string") && (url.length > 0) ){
			...
			$.ajax({
				...
				success: function(data) {
					...
					if(parsed.status==-1){
						toastr.error(data.message || "请求信息错误，请刷新！", opts);
						return;
					}
					if(parsed.status==-2){//身份认证错误，需要重新登录
						toastr.error(parsed.message || "您的信息认证未通过，请重新认证！", opts);
						return;
					}
	...
	function sayHello(data){
        ...
        if(data.status==-1){
			toastr.error("网络有点不给力，请刷新页面尝试。", opts);
            return;
        }
        if(data.status==-2){//身份认证错误，需要重新登录
			toastr.error("您的信息认证未通过，请重新认证。", opts);
        ...
        $("#"+options.clearScreen).click(function(){
            var welcomP= $("<div/>").addClass("staff-msg");
	 ...
	 //流程问题的查看
	 $('#'+options.chatContDiv).on('click','.wflink',function() {
        $this = $(this);
	...
	//接入人工
	 $('#'+options.chatContDiv).on('click','.faqevent',function() {
    ...
    function init_leadQuestions(data){
        ...
        //默认网页欢迎语优先
        if(data.helloWord){
            robot_icon_small.html(data.helloWord);
        }else{
            if(data.webConfig.helloWord.indexOf(data.webConfig.robotName)>-1){
                robot_icon_small.html(data.webConfig.helloWord);
            }else{
                robot_icon_small.html('您好，我是机器人'+data.webConfig.robotName+'。'+data.webConfig.helloWord);
            }  
        }
...
break;
    ...
    function createMyWordsHtml(msg,time){
		var myword='<div  class="client-msg" data-type="text"><div class="logoSize" style="right:0"><img class="img-circle" alt="" src="images/head_user.png"></div><div class="msg rightLogo"><div class="content">'+msg+'</div><span class="msg-status"></span> </div> </div>';
        return myword;
    }
	...
	function createRobotWordsHtmlItem(ansItem,time){
		...
		if(typeof ansItem.relateLessList !='undefined' && ansItem.relateLessList!="" && ansItem.relateLessList.length>0){
			$('#'+options.sugQuestionDiv).html('');
			formatQueULList(options.sugQuestionDiv,ansItem.relateLessList);
		}
		...
		//推荐问题
		if(typeof ansItem.relateList!="undefined" && ansItem.relateList.length>0){
        ...
        if(typeof ansItem.gusList!="undefined" && ansItem.gusList.length>0){
            var  lielement = $("<span/>").addClass('question').html(beforeWords).appendTo(robot_icon_more);
			var ul_Element=$('<ol/>').appendTo(robot_icon_more);
    ...
    function offLine(){
       var s_q_url=options.basePath+'AQ?s=offline&nocache='+new Date().getTime();
	    $.getJSON(
            ...
            function(data){
				if(data.status==0){
					var obj={"message" : "","robotAnswer" : [{"ansCon" : "您的会话已过期，如需再次聊天，请重新打开或刷新本页面！"}],"status" : 0}
   ...
   //字数统计
	...
	function helpclick(){
	 ...
	 function showReason(aid){
		...
		tmpObj.html(init_unsolvedReason(options.unsolvedReason));
		tmpObj.find('.subreasonForm').attr('cluid',aid);
	 }
	 //提交未解决的原因
	  chatContDIv.on('click','.subreasonForm',function(){
		  var tmpId=$(this).attr('cluid');
		  var reasonType=''
		  ...
		  $.ajax({
				...
				success:
					function(data){
						if(data.status==0){
							$('.helper_aid_'+tmpId).html('<div style="color:#666;">感谢您的反馈</div>');
						}else{
...
$.FaqRobot.defaults = {
	...
	kfUrl:'images/head.png',  //客服图标
	khUrl:'images/head_user.png', //客户图标
