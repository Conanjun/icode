...
(function () {
    iframeTab = jQuery.prototype = {
        ...
        init: function (option) {
            ...
            function setBreadcrumb(iframe){//配置面包屑
                /*
                * taskId = 747 动态配置页面标题和面包屑 by 司徒棋巽
                */
                setTimeout(function(){
                    if(iframe.parent().hasClass('active')){
                        var firstLevel = iframe.attr('data-firstlevel')
                        var secondlevel = iframe.attr('data-secondlevel')
                        var breadcrumb = ""
                        if(firstLevel == secondlevel){//若一级和二级菜单相同，则面包屑只需一层
                            breadcrumb = '<li class="active">' + firstLevel + '</li>';
                        }else{//否则，面包屑统一两层
                            breadcrumb = '<li><a href="javascript:;">' + firstLevel + '</a></li><li class="active">' + secondlevel + '</li>'
                        }
                        iframe.contents().find('body .breadcrumb').html(breadcrumb);
                        if(iframe.contents().find('body .page-header').children('span.page-header-span')){
                            iframe.contents().find('body .page-header').children('span.page-header-span').html(secondlevel);
                        }else{
                            iframe.contents().find('body .page-header').html(secondlevel);
                        }
            ...
            function stellung (event) { // 創建窗口
                if(window.top.location.href.indexOf("web")>-1){
                    ...
                    var $this = $(this),//点击可创建窗口的a标签
                        ...
                        * taskId = 747 动态配置页面标题和面包屑 by 司徒棋巽
                        */
                        firstLevel = $this.attr('data-firstLevel')||name,//一级菜单名称
                        secondLevel = $this.attr('data-secondLevel')||name,//二级菜单名称
                        ...
                        * taskId = 747 动态配置页面标题和面包屑 by 司徒棋巽
                        */
                        $iframe = $('<iframe src="' + link + '" data-firstLevel="' + firstLevel + '" data-secondLevel="' + secondLevel + '" data-iframe="' + link + '" data-num="' + date + '" marginheight="0" marginwidth="0" frameborder="0" scrolling="auto"></iframe>');
                    ...
                    setBreadcrumb($tabBody.find('iframe[data-iframe="' + link + '"]'));
                    return false;
                }
            }
            ...
            tab = {
                ...
                closeActIframe: function (isRefresh,comeUrl) {
                    if(window.top.location.href == window.location.href) {//判断是否为单独页面
                        ...
                        /*
                        * taskId =471  页面自动关闭问题
                        * 用操作确定的页面和所有的li进行对比，如果匹配到li属性data-tab和路径相等的则操作关闭此页面
                        * */
                        for(var i = 0;i < parent.$('#tabHeader').find('li').length;i++){
                            if(parent.$('#tabHeader').find('li').eq(i).attr('data-tab') == window.location.href){
                                var actLi = parent.$('#tabHeader').find('li').eq(i);
                            }
                        }
                        if (!isRefresh) {
                            if(comeUrl){
