...
if (getUrlParam('tsFlag')) {
    ...
    } else if (tsJSON.Mode == '10') {
        $('#AnswerNav a[href="#tabArray"]').tab('show');
        $('#tabId').val(tsJSON.ModeValue);
        if (data.thirdOpen !== undefined) {
            $('#ans_tab button').html(data.thirdOpen.Info);
        }
...
$(document).ready(function () {
    ...
    $(document).on('blur', '[name="form_Question"]', function () {
        ...
        //判断该问题是否在相似问法中存在
        $('[name=similarQueInput]').each(function (i, item) {
            if (question && $(item).val().trim() == question) {
                yunNotyError("该问题已存在！");
                return;
            }
        });
    ...
    $('#addSimilarInput').click(function (e) {
        ...
        */
        ...
        //判断是否与当前创建的问题重复
        if(question && question == $('[name="form_Question"]').val()){
...
function getInfo() {
    $('#queGroup input[data-change="check-switchery-state-text"]').unbind('change').bind("change", function () {
        if ($(this).attr('checked')) {
            //启用
            $("#queManual").addClass('active in');
        } else {
            $("#queManual").removeClass('active in');
        }
    });
    $('#queManual').empty();
    for (var p = 0; p < 5; p++) {
...
function clearMagnet() {
    ...
    $('#addSimilarInput').parent().before('<div class="form-group"><div class="col-md-offset-2 col-md-8 input-group"><input class="form-control" type="text" placeholder="请填写相似问法" name="similarQueInput" maxlength="200"><a href="javascript:;" name="delSimilarInput" class="input-group-addon"><span class="fa fa-times"></span></a></div></div>');
    $('#similarQueModal').modal('hide');
            ...
            if (data.status === 0) {
                if (data.list.length > 0) {
                    ...
                    for (var i = 0; i < data.list.length; i++) {
                        html += "<tr id=\"list-tr-" + data.list[i].Id + "\" hits='" + data.list[i].Hits + "'>";
                    ...
                    if ($('#search_Voice input[name=name]').val() !== '') {
                        ...
                        $('#voiceList').find('tbody').html('<tr><td colspan=\"3\"  style=\"text-align:center;\"><i class=\"glyphicon glyphicon-warning-sign\"></i>&nbsp;&nbsp;当前纪录为空！<br><a href="../../web/material/addSrc.html" data-parent="2" data-me="00" data-name="添加素材" data-num="0" data-reload="ja">点击跳转到添加素材页面</a></td></tr>');
                    }
                    $('#voicepageList').html('');
                }
            } else {
...
$('#selOtherBtn').click(function () {
    ...
    $('#otherId').val(id);
    $('#ans_other button').html($('#otherDiv #list-tr-' + id).children('td').eq(1).html());
    if ($('#ans_other button').html() === '') {
        $('#ans_other button').hide();
    } else {
        $('#ans_other button').show();
...
function tabArrayThead() {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.list) {
                tabStr = '<a class="tabTitle btn btn-primary m-r-5 m-t-5" tabId="" href="javascript:void(0)">' + data.main + '</a>' + '： ';
                $.each(data.list, function (i, item) {
                    if (item.selecting) {
                        tabStr += '<a class="btn btn-primary m-r-5 m-t-5" tabId="' + i + '" tat="0">' + item.xson + '</a>';
            ...
            if (json.status === 0) {
                if (json.list.length === 0) {
                    ...
                    for (var i in json.list) {  //图片生成
                        ...
                        var img = $("<img src='" + (localStorage.getItem('Subdomain')||"") + "/" + json.list[i].Path + "' />").appendTo(li);
                    ...
                    $(".onlyImgLi").each(function () {
                        $(this).click(function () {
                            ...
                            var src = $(this).data("src");
                            $("#OnlyPictureId").prevAll().children("img").attr("src", (localStorage.getItem('Subdomain')||"") + "/" + src);
...
function Add() {
    ...
    if ($('#termsIds').val() != '') {
        if (getUrlParam('tsFlag')) {
            ...
            //fd.append('termsIds',$('#termsIds').val());
        }
    }
                ...
                if (getUrlParam('tsFlag')) {
                    ...
                    yunNoty(data, function () {
                        ...
                        if (!isAgain) {
                            ...
                            // 默认选择纯文本标签
                            $("#AnswerNav").find("li").removeClass("active")
                            $("#AnswerNav").find("li:first").addClass("active")
                            $("#AnswerNav").next().find(".active").removeClass("active in")
                            $("#text").addClass("active in")
                            //清空表单
                            $(".formSubmit").attr("disabled", false)
                            $('#question').val('');
                            $("#EntitysList .entitys").val('');
                            $('#ans_text').val('');
                            delImg();
                            UE.getEditor('ans_richtext').setContent('');
                            $('#pictureShow').html('').css('display', 'none');
                            $("#delPicture").parent().remove();
                            $('#pictureId').val('');
                            $('#ans_voice button').hide();
                            $('#voiceId').val('');
                            $('#ans_other button').hide();
                            ...
                            $('#askId').val('');
                            $('#ans_tab button').hide();
                            $('#tabArray').find("#tabArrayThead").html('');
                            $('#tabName').val('');
                            $('#tabId').val('');
                            $('#ans_form button').hide();
                            $('#formId').val('');
